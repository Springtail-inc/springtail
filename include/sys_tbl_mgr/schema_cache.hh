...
/**
 * A class for holding schema metadata from the sys_tbl_mgr::Service.  It currently only caches the
 * full schema metadata.
 *
 * Originally I had planned to also cache the history of changes, but integrating the history of
 * changes to identify gaps and connect them to the correct base schemas was turning into a lot of
 * complexity.  Instead, I would suggest that we replace the entire SysTblMgr with a cache of system
 * table rows that works using the WriteCache.  Changes that haven't been sync'd to disk can be
 * merged into the results, allowing each node in the system to locally cache whatever schema and
 * change information it needs without contacting a centralized SysTblMgr service.
 *
 * Once that change is made, if needed, we can expand this to layer a more complex cache of
 * constructed objects on top of that, but it becomes a lot cheaper to construct missing metadata
 * since the centralized page cache will hold the recently used data we need.
 */
class SchemaCache {
public:
...
public:
    explicit SchemaCache(int capacity) : _capacity(capacity) {}
...
private:
    /**
     * Update the internal map from index id -> table id.
     */
    void _populate_index_map_locked(uint64_t db, uint64_t tid, const std::vector<Index> &indexes);
    ...
    /**
     * Removes the provided schema_map entry from the cache.
     */
    void _remove_locked(SchemaMap::iterator schema_i);

    /**
     * Makes space for a new cache entry, releasing the LRU entry if needed to generate capacity.
     */
    void _make_space_locked();

private:
    std::mutex _mutex; ///< Protects the private members
