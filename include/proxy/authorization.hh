...
namespace springtail::pg_proxy {
    ...
    public:
        ClientAuthorization(ProxyConnectionPtr connection,
                            ...
                            int32_t pid,
                            int32_t cancel_key)
            : _state(STARTUP), _connection(connection), _id(id),
              _pid(pid), _cancel_key(cancel_key)
        {}
    ...
    public:
        ServerAuthorization(ProxyConnectionPtr connection,
                            ...
                            Session::Type type,
                            const std::unordered_map<std::string, std::string> &parameters)
            : _state(STARTUP), _connection(connection), _id(id), _user(user),
              _database(database), _db_prefix(db_prefix),
              _login(user->get_user_login()), _type(type), _parameters(parameters)
        {}
