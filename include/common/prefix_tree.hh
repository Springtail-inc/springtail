...
template <typename T> class PrefixNode {
  public:
     ...
     *      NOTE: the values stored at the node level won't be compared to the
     *          ones that are already stored there. If the same value is added more than
     *          once, then it will be there as many times as it has been added and also
     *          would have to be removed as many times as it has been added
     *
     * @param node_path - queue of strings representing the path down from this
     *                    node
     ...
     *          the code of this function recursively walks the path till
     *          it reaches the level at which the data should be stored.
     *          If the path is wrong and points to non-existing branches,
     *          nothing will will be removed.
     *
     * @param node_path
     * @param value
     */
    ...
    /**
     * @brief Collect the stored items from the tree in a vector (queue) of
     * pairs that contain a path where the value item was found and the value
     * item itself. The item selection is based on criteria implemented in
     * select function.
     *
     * @param out_queue - the queue of path and value item pairs
     * @param path - prefix path to use on each level
     * @param node_path - the path that specifies where the value items should
     *      come from as long as the path is not empty, the value items will be;
     *      collected at the nodes visited by the path; if the
     *       path is empty, then all the nodes under this path will be visited
     * @param select_fun - function for selection criteria
     */
    void collect_items(
        std::vector<std::pair<std::string, T>> &out_queue,
        const std::string &path, std::deque<std::string> &node_path,
        ...
        if (node_path.empty()) {
            ...
            for (const auto &child : _children) {
                ...
                child.second->collect_items(out_queue, next_path, node_path,
                                            select_fun);
            }
        } else {
            ...
            if (_children.contains(top_key)) {
                ...
                _children[top_key]->collect_items(out_queue, next_path,
                                                  node_path, select_fun);
            }
        }
    }

    /**
     * @brief Count the number of items stored in the tree that satisfy
     * selection criteria
     *
     * @param node_path - path to follow through the tree
     * @param path - prefix path to use on each level
     * @param select_fun - function for selection criteria
     * @return size_t - number of items on the path and under it that satisfy
     * selectio criteria
     */
    size_t count_items(
        std::deque<std::string> &node_path, const std::string &path,
        std::function<bool(const std::string &, const T &)> select_fun) {
        size_t count = 0;
        for (const auto &value : _values) {
            if (select_fun(path, value)) {
        ...
        if (node_path.empty()) {
            ...
            for (const auto &child : _children) {
                std::string next_path = path + "/" + child.first;
                count +=
                    child.second->count_items(node_path, next_path, select_fun);
            }
        } else {
            // as long as the in_queue is not empty, follow the path
            std::string top_key = node_path.front();
            ...
            if (_children.contains(top_key)) {
                std::string next_path = path + "/" + top_key;
                count += _children[top_key]->count_items(node_path, next_path,
                                                         select_fun);
            }
        }
        return count;
    ...
    size_t size() {
        ...
        return count_items(node_path,
                           [](const std::string &, const T &) { return true; });
    }

  private:
    /**
     * @brief Map of branches from the current node. It maps a name to on of the
     * child nodes
     *
     */
