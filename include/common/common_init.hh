...
namespace springtail::init {
    ...
    public:
        PropertiesRunner(bool load_redis) :
                ServiceRunner("Properties"), _load_redis(load_redis) {}
        bool start() override {
    ...
    public:
        TracingRunner(const std::optional<std::string> &name) :
                ServiceRunner("Tracing"), _name(name) {}
        bool start() override {
            tracing::TracingAndMetrics::get_instance()->init(_name.value_or(""));
            return true;
        }
        void stop() override {
            tracing::TracingAndMetrics::shutdown();
        }
    private:
