...
namespace springtail {
    ...
    public:
        ...
        ExtentHeader(std::shared_ptr<std::vector<char>> data)
        {
            ...
            std::memcpy(&xid, data->data() + 1, sizeof(uint64_t));
            std::memcpy(&row_size, data->data() + 9, sizeof(uint32_t));
            std::memcpy(&prev_offset, data->data() + 13, sizeof(uint64_t));
            std::memcpy(&field_count, data->data() + 21, sizeof(uint32_t));
            field_types.resize(field_count);
            std::memcpy(field_types.data(), data->data() + 25, field_count);
        }
        ...
        std::vector<char> pack() const
        {
            ...
            *reinterpret_cast<uint8_t *>(data.data()) = static_cast<uint8_t>(type);

            std::memcpy(data.data() + 1, &xid, sizeof(uint64_t));
            std::memcpy(data.data() + 9, &row_size, sizeof(uint32_t));
            std::memcpy(data.data() + 13, &prev_offset, sizeof(uint64_t));
            std::memcpy(data.data() + 21, &field_count, sizeof(uint32_t));
            std::memcpy(data.data() + 25, field_types.data(), field_count);
