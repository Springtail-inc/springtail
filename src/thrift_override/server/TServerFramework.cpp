...
#include <stdexcept>
#include <stdint.h>
#include <thrift_override/server/TServerFramework.h>
...
int64_t TServerFramework::getConcurrentClientLimit() {
  std::unique_lock lock(mutex_);
  return limit_;
}

int64_t TServerFramework::getConcurrentClientCount() {
  std::unique_lock lock(mutex_);
  return clients_;
}

int64_t TServerFramework::getConcurrentClientCountHWM() {
  std::unique_lock lock(mutex_);
  return hwm_;
}
