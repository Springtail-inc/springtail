...
void TServerSocket::listen() {
  ...
  if (isUnixDomainSocket()) {
    ...
    do {
      ...
      GlobalOutput.printf("Opened listening server socket = %d", serverSocket_);
      _setup_sockopts();
      _setup_tcp_sockopts();
      ...
      if (0 == ::bind(serverSocket_, trybind->ai_addr, static_cast<int>(trybind->ai_addrlen))) {
        GlobalOutput.printf("Bound listening server socket = %d", serverSocket_);
        break;
      }
      errno_copy = THRIFT_GET_SOCKET_ERROR;
...
shared_ptr<TTransport> TServerSocket::acceptImpl() {
  ...
  GlobalOutput.printf("Accepted client socket = %d with sendTimeout_ = %d, recvTimeout_ = %d",
    clientSocket, sendTimeout_, recvTimeout_);
