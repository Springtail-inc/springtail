...
Services::Services(bool xid_mgr, bool sys_tbl_mgr, bool write_cache)
    : _xid_mgr(xid_mgr), _sys_tbl_mgr(sys_tbl_mgr), _write_cache(write_cache)
{
}
...
Services::init()
{
    ...
    if (_xid_mgr) {
        ...
        */
        ...
        _threads.push_back(std::thread([] { xid_mgr::XidMgrServer::get_instance()->startup(); }));
    }
    ...
    if (_sys_tbl_mgr) {
        ...
        _threads.push_back(std::thread([] { sys_tbl_mgr::Server::get_instance()->startup(); }));
    }

    // start the Write Cache
    if (_write_cache) {
        _threads.push_back(std::thread([] { WriteCacheServer::get_instance()->startup(); }));
    }
