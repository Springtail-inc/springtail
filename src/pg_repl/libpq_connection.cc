 ...
 */
void
LibPqConnection::start_transaction()
{
    exec(BEGIN_QUERY);
    clear();
 ...
 */
void
LibPqConnection::end_transaction()
{
    exec(END_QUERY);
    clear();
 ...
 */
int
LibPqConnection::get_copy_data(bool async)
{
 ...
 */
char *
LibPqConnection::get_copy_buffer()
{
    return _buffer;
}
 ...
 */
void
LibPqConnection::free_copy_buffer()
{
    if (_buffer == nullptr) {
        return;
    }
 ...
 */
void
LibPqConnection::exec(const std::string &cmd)
{
    exec(cmd.c_str());
}
 ...
 */
void
LibPqConnection::exec(const char *cmd)
{
    if (_connection == nullptr) {
        throw PgNotConnectedError();
    }
    ...
    if (PQresultStatus(res) != PGRES_COMMAND_OK && PQresultStatus(res) != PGRES_TUPLES_OK &&
        ...
        std::string error_message = fmt::format("msg={}, status={}", PQerrorMessage(_connection),
 ...
 */
int32_t
LibPqConnection::get_int32(int row, int col)
{
...
int64_t
LibPqConnection::get_int64(int row, int col)
{
    char *value = PQgetvalue(_result, row, col);
    if (value == nullptr) {
        throw PgQueryError();
 ...
 */
std::string
LibPqConnection::get_string(int row, int col)
{
    char *value = PQgetvalue(_result, row, col);
    if (value == nullptr) {
        throw PgQueryError();
...
LibPqConnection::get_string_optional(int row, int col)
{
    char *value = PQgetvalue(_result, row, col);
    if (value == nullptr) {
        throw PgQueryError();
 ...
 */
bool
LibPqConnection::get_boolean(int row, int col)
{
    char *value = PQgetvalue(_result, row, col);
    if (value == nullptr) {
        throw PgQueryError();
 ...
 */
std::string
LibPqConnection::escape_string(const std::string &str)
{
    char new_str[str.length() * 2 + 1];
    if (_connection == nullptr) {
        PQescapeString(new_str, str.c_str(), str.length());
    } else {
        PQescapeStringConn(const_cast<PGconn *>(_connection), new_str, str.c_str(), str.length(),
                           nullptr);
    }
...
std::string
LibPqConnection::_escape_identifier(const char *input)
{
...
std::string
LibPqConnection::escape_identifier(const std::string &str)
{
    if (_connection == nullptr) {
 ...
 */
void
LibPqConnection::connect(const std::string &db_host,
                         ...
                         const bool replication)
{
    ...
    while (retries < MAX_RETRY_COUNT) {
        ...
        std::string conninfo = fmt::format(
            ...
            hosttype, host, db_port, name, user, pass, (replication ? "replication=database " : ""),
            encoding);
        ...
        if (PQstatus(connection) != CONNECTION_OK) {
            SPDLOG_ERROR("Error connecting: conninfo: {}, msg: {}", conninfo,
...
int
LibPqConnection::wait_until_readable(int timeout_sec)
{
    assert(_is_replication);
...
int
LibPqConnection::wait_until_writable(int timeout_sec)
{
    assert(_is_replication);
...
ssize_t
LibPqConnection::read(char *buf, size_t count, bool async)
{
    if (_connection == nullptr) {
        throw PgNotConnectedError();
    }
...
ssize_t
LibPqConnection::write(const char *buf, size_t count, bool async)
{
    if (_connection == nullptr) {
        throw PgNotConnectedError();
    }
 ...
 */
char *
LibPqConnection::get_value(int row, int col)
{
    if (_result == nullptr) {
        return nullptr;
    }
 ...
 */
int
LibPqConnection::length(int row, int col)
{
    if (_result == nullptr) {
        return 0;
    }
 ...
 */
int
LibPqConnection::server_version()
{
    if (_connection == nullptr) {
        throw PgNotConnectedError();
    }
 ...
 */
int
LibPqConnection::put_copy_end(const char *errormsg)
{
    if (_connection == nullptr) {
        throw PgNotConnectedError();
    }
