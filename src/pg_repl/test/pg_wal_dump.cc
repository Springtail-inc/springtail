...
int main(int argc, char* argv[])
{
    ...
    LOG_INFO(LOG_ALL, "Connection and streaming have started @ LSN={}.  Dumping data.\n", lsn);
    PgCopyData data;
    bool skip = true;
    ...
    while (true) {
        ...
        LOG_INFO(LOG_ALL, "Recevied data: data len={}, msg len={}, msg offset={}\n",
                     data.length, data.msg_length, data.msg_offset);
        LOG_INFO(LOG_ALL, "  - start LSN={}, end LSN={}\n",
        ...
        if (data.msg_offset + data.length == data.msg_length) {
            if (true || !skip) {
                LOG_INFO(LOG_ALL, "Setting last flushed LSN: {}\n", data.ending_lsn);
