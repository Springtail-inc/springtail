 ...
 */
void
PgCopyTable::connect(const std::string &hostname,
...
void
PgCopyTable::_get_secondary_indexes()
{
...
void
PgCopyTable::_set_schema(const std::string &table_name,
                         ...
                         uint64_t table_oid)
{
    ...
    try {
        ...
        for (int i = 0; i < rows; i++) {
            ...
            SPDLOG_DEBUG_MODULE(
                LOG_PG_REPL, "Column: {} type={} position={} nullable={} default_value={} pkey={}",
                         ...
                         uint64_t table_oid)
{
    ...
    auto table =
        TableMgr::get_instance()->get_snapshot_table(db_id, _schema.table_oid, xid.xid, schema);
...
int32_t
PgCopyTable::_verify_copy_header(const std::string_view &header)
{
    // verify signature
    ...
    for (int i = 0; i < num_columns; i++) {
        ...
        switch (type) {
            case (SchemaType::TEXT):
                fields->push_back(std::make_shared<ConstTypeField<std::string>>(
                    std::string(row.data() + pos, length)));
                pos += length;
                break;
            ...
            case (SchemaType::INT64):
                CHECK_EQ(length, 8);
                fields->push_back(
                    std::make_shared<ConstTypeField<int64_t>>(recvint64(row.data() + pos)));
                pos += length;
                break;
            ...
            case (SchemaType::INT32):
                CHECK_EQ(length, 4);
                fields->push_back(
                    std::make_shared<ConstTypeField<int32_t>>(recvint32(row.data() + pos)));
                pos += length;
                break;
            ...
            case (SchemaType::INT16):
                CHECK_EQ(length, 2);
                fields->push_back(
                    std::make_shared<ConstTypeField<int16_t>>(recvint16(row.data() + pos)));
                pos += length;
                break;
            ...
            case (SchemaType::INT8):
                CHECK_EQ(length, 1);
                fields->push_back(
                    std::make_shared<ConstTypeField<int8_t>>(recvint8(row.data() + pos)));
                ++pos;
                break;
            ...
            case (SchemaType::BINARY): {
                ...
                SPDLOG_WARN("Converting unsupported type '{}' into BINARY -- {}", pg_type, tmp);
            ...
            default:
                throw TypeError(
                    fmt::format("PG doesn't support type: {}", static_cast<uint8_t>(type)));
        }
    }
                             ...
                             std::vector<std::tuple<std::string, std::string, int32_t>> &table_oids)
{
    ...
    if (include_json.contains("schemas") && include_json["schemas"].is_array()) {
        ...
        if (!schemas.empty()) {
            ...
            _get_table_oids(
                fmt::format(TABLES_SCHEMA_QUERY,
    ...
    if (include_json.contains("tables") && include_json["tables"].is_array()) {
        ...
        for (const auto &table : include_json["tables"]) {
            if (table.contains("schema") && table.contains("table")) {
                ...
                pairs.push_back(fmt::format("('{}', '{}')", _connection.escape_string(schema),
                                            _connection.escape_string(table_name)));
            }
        }

        if (!pairs.empty()) {
            // issue query by joining all the schema, table pairs
            _get_table_oids(fmt::format(TABLE_SCHEMA_PAIR_QUERY,
                                        common::join_string(",", pairs.begin(), pairs.end())),
                            table_oids);
        }
    }
}
...
PgCopyTable::copy_tables(uint64_t db_id, uint64_t xid, std::vector<uint32_t> table_oids)
{
    return _internal_copy(db_id, xid, std::nullopt, std::nullopt, table_oids);
}

std::vector<PgCopyResultPtr>
PgCopyTable::copy_schema(uint64_t db_id, uint64_t xid, const std::string &schema_name)
{
...
std::vector<PgCopyResultPtr>
PgCopyTable::copy_table(uint64_t db_id, uint64_t xid, const nlohmann::json &include_json)
{
                     ...
                     PgCopyResultPtr result)
{
    ...
    while (true) {
        ...
        try {
            // copy the table
            copy_table._copy_table(db_id, xid, request->table_name, request->schema_name,
                                   request->table_oid);
...
{
    std::string sync_msg =
        fmt::format(R"({{"target_xid":{}, "pg_xid":{}}})", result->target_xid, result->pg_xid);
                            ...
                            std::optional<nlohmann::json> include_json)
{
    ...
    if (schema_name.has_value()) {
        ...
        // by table oids
        std::string tids =
            common::join_string(",", table_tids.value().begin(), table_tids.value().end());
    ...
    for (int i = 0; i < std::min(static_cast<std::size_t>(WORKER_THREADS), table_oids.size());
         i++) {
        PgCopyResultPtr copy_result = std::make_shared<PgCopyResult>(target_xid);
        table_results.push_back(copy_result);
        workers.push_back(std::thread(&PgCopyTable::_worker, &copy_table, db_id, target_xid,
                                      copy_queue, copy_result));
    }
