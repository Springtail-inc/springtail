...
PgMsgLogGen::~PgMsgLogGen() { ::fclose(_fp); }
...
PgMsgLogGen::_gen_table_schema(uint32_t table_id, const std::vector<PgMsgSchemaColumn> &columns)
{
    ...
    for (auto &&c : columns) {
        ...
        assert(c.pg_type == BOOLOID || c.pg_type == INT4OID || c.pg_type == INT8OID ||
               c.pg_type == TEXTOID);
...
uint32_t
PgMsgLogGen::create_index(const std::string &index,
                          const std::string &table_name,
                          uint32_t table_oid,
                          const std::vector<PgMsgSchemaIndexColumn> &columns)
{
...
void
PgMsgLogGen::delrow(uint32_t table_id, const std::vector<std::string> &key_columns, bool using_pkey)
{
