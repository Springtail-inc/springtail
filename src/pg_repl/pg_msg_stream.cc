...
PgMsgStreamReader::_read_header()
{
    ...
    if (header.magic != PgMsgStreamHeader::PG_LOG_MAGIC) {
        SPDLOG_WARN("Invalid stream header magic number: {}, offset: {}", header.magic,
                    _current_offset);
        throw PgIOError();
    }

    SPDLOG_DEBUG_MODULE(LOG_PG_REPL, "Reading header at offset: {}, msg_length: {}", _header_offset,
                        header.msg_length);
...
PgMsgPtr
PgMsgStreamReader::read_message(const std::vector<char> &filter, bool &eos, bool &eob)
{
...
PgMsgStreamReader::read_message(const std::vector<char> &filter)
{
    ...
    try {
        ...
        SPDLOG_DEBUG_MODULE(
            LOG_PG_REPL,
            "Reading message type: {}, current_offset: {}, end_offset: {}, skip_msg: {}", msg_type,
            _current_offset, _end_offset, skip_msg);
...
PgMsgStreamReader::_decode_tuple(PgMsgTupleData &tuple)
{
    ...
    for (int i = 0; i < num_columns; i++) {
        ...
        if (tuple.tuple_data[i].type == 'n' || tuple.tuple_data[i].type == 'u') {
            continue;
        }
...
PgMsgStreamReader::_decode_relation()
{
    /*
        ...
        Int8 Replica identity setting for the relation (same as relreplident in pg_class).
          ...
          # background:
       https://www.postgresql.org/docs/10/sql-altertable.html#SQL-CREATETABLE-REPLICA-IDENTITY Int16
       Number of columns. Next, the following message part appears for each column (except generated
       columns): Int8 Flags for the column. Currently can be either 0 for no flags or 1 which marks
...
PgMsgStreamReader::_decode_update()
{
    /*
        ...
        TupleData       TupleData message part representing the contents of the old tuple or primary
       key. Only present if the previous 'O' or 'K' part is present. Byte1('N')      Identifies the
       following TupleData message as a new tuple. TupleData       TupleData message part
       representing the contents of a new tuple.
...
PgMsgStreamReader::_decode_delete()
{
    /*
        ...
        Int32           ID of the relation corresponding to the ID in the relation message.
        Byte1('K')      Identifies the following TupleData submessage as a key.
                        This field is present if the table in which the delete has happened uses an
       index as REPLICA IDENTITY. Byte1('O')      Identifies the following TupleData message as a
       old tuple. This field is present if the table in which the delete has happened has REPLICA
       IDENTITY set to FULL. TupleData       TupleData message part representing the contents of the
       old tuple or primary key, depending on the previous field.
...
PgMsgStreamReader::_decode_stream_start()
{
    /*
        ...
        Int32       Xid of the transaction.
        Int8_t      A value of 1 indicates this is the first stream segment for this XID, 0 for any
       other stream segment.
    */
...
PgMsgStreamReader::_decode_stream_abort()
{
    /*
        ...
        Int32       Xid of the transaction.
        Int32       Xid of the subtransaction (will be same as xid of the transaction for top-level
       transactions). Int64       The LSN of the abort. This field is available since protocol
       version 4. Int64       Abort timestamp of the transaction. The value is in number of
...
PgMsgStreamReader::scan_log(const std::filesystem::path &file, bool truncate)
{
    ...
    while (true) {
        ...
        SPDLOG_DEBUG_MODULE(LOG_PG_REPL,
                            "Header: start_lsn: {}, end_lsn: {}, msg_length: {}, msg_type: {}",
...
PgMsgStreamWriter::PgMsgStreamWriter(const std::filesystem::path &file) : _file(file)
{
