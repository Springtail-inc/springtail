...
static constexpr char CONFIRMED_FLUSH_LSN_SQL[] =
...
PgReplConnection::~PgReplConnection() { close(); }
...
PgReplConnection::start_streaming(LSN_t LSN)
{
    ...
    if (LSN == INVALID_LSN) {
        // execute query: SELECT confirmed_flush_lsn FROM pg_replication_slots WHERE slot_name =
        // 'slot_name';
        _connection->exec(fmt::format(CONFIRMED_FLUSH_LSN_SQL, _slot_name));

        // process results; sanity checks first
        if (_connection->status() != PGRES_TUPLES_OK || _connection->ntuples() <= 0 ||
            _connection->length(0, 0) < 1) {
...
void
PgReplConnection::_send_copy_data(const char *buffer, int length, char cmd = MSG_COPY_DATA)
{
...
int
PgReplConnection::_recv_copy_data(char *buffer, int length, bool async = true)
{
    while (!_shutdown) {
...
PgReplConnection::_read_msg_header()
{
    ...
    while (msg_header_offset < COPY_MSG_HDR_SIZE) {
        ...
        int r =
            _recv_copy_data(&msg_header[msg_header_offset], COPY_MSG_HDR_SIZE - msg_header_offset);
...
{
    int to_read = std::min(_copy_msg_length - _copy_msg_offset, COPY_BUFFER_SIZE);
    ...
    while (to_read > 0) {
        ...
        // message
        int r = _recv_copy_data(&_copy_buffer[offset], to_read, async);
        to_read -= r;
        length += r;
        offset += r;
    }
...
PgReplConnection::read_data(PgCopyData &dataOut)
{
    ...
    if (_copy_state == READ_COPY_HEADER || _copy_state == STREAMING) {
        _read_copy_data();
    }
...
PgReplConnection::_process_keep_alive(const char *buffer, int length)
{
    ...
    SPDLOG_DEBUG_MODULE(LOG_PG_REPL,
                        "Keep alive msg recvd: wal_end={}, send_time={}, last_flushed LSN={}",
...
PgReplConnection::_process_xlog_header(const char *buffer, int length)
{
    ...
    // see
    // https://github.com/postgres/postgres/blob/f234b8cd16a4ba6e12cc51a36c8e499661d535bb/src/backend/replication/walreceiver.c#L1256
...
int
PgReplConnection::_encode_standby_status_msg(int64_t send_time, char replybuf[34])
{
    int pos = 0;
...
bool
PgReplConnection::check_slot_exists(LSN_t &restart_lsn_out, LSN_t &flushed_lsn_out)
{
    if (_started_streaming) {
    ...
    std::string cmd = fmt::format(
        "SELECT restart_lsn, confirmed_flush_lsn from pg_catalog.pg_replication_slots WHERE "
        "slot_name='{}'",
        slot_name);
    ...
    // process results
    if (_connection->status() != PGRES_COMMAND_OK && _connection->status() != PGRES_TUPLES_OK) {
...
void
PgReplConnection::create_replication_slot(bool export_snapshot, bool temporary)
{
    if (_started_streaming) {
        throw PgStreamingError();
    }
    ...
    if (_connection->ntuples() != 1 || _connection->nfields() != 4) {
        SPDLOG_ERROR(
            "Unexpected number of rows or columns for CREATE REPLICATION SLOT: rows={}, cols={}\n",
