    ...
    for (auto &&pair : columns) {
        ...
        switch (column.type) {
            ...
            case (SchemaType::BOOLEAN): {
                ...
                field =
                    std::make_shared<ExtentField>(column.type, (bool_pos >> 3), (bool_pos & 0x7));
                            ...
                            const std::vector<std::string> &sort_columns) const
{
    ...
    for (auto &&name : old_columns) {
        ...
        if (pos == sort_columns.end()) {
            all_columns.push_back({name, size, i->second.first->get_type(),
                                   _pg_types[i->second.second], i->second.first->can_be_null()});
        } else {
            all_columns.push_back({name, size, i->second.first->get_type(),
                                   _pg_types[i->second.second], i->second.first->can_be_null(),
