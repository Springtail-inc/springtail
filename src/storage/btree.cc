...
BTree::BTree(const std::filesystem::path &file,
    ...
    : _file(file), _xid(xid), _leaf_schema(schema), _root_offset(root_offset)
{
...
BTree::Iterator
BTree::lower_bound(TuplePtr search_key, bool for_update) const
{
    // check if we don't have a root
    if (_root_offset == constant::UNKNOWN_EXTENT) {
        return end();
...
BTree::Iterator
BTree::upper_bound(TuplePtr search_key, bool for_update) const
{
    // check if we don't have a root
    if (_root_offset == constant::UNKNOWN_EXTENT) {
        return end();
