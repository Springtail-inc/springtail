...
public:
    ...
    void TearDown() { std::filesystem::remove_all(_base_dir); }
    ...
    void populate_btree_many_rows(MutableBTreePtr btree, int row_count)
    {
        ...
        for (int i = 0; i < row_count; i++) {
            ...
            (*key_fields)[1] =
                std::make_shared<ConstTypeField<std::string>>(fmt::format("name_{}", i));
    ...
    std::shared_ptr<MutableBTree> create_mutable_btree(const std::filesystem::path& name,
                                                       uint64_t xid)
    {
...
static void
BM_BTreeLookupHelper(benchmark::State& state, bool miss)
{
    ...
    for (auto _ : state) {
        ...
        key_fields->at(0) =
            std::make_shared<ConstTypeField<std::string>>(fmt::format("name_{}", to_lookup));
...
static void
BM_BTreeLookupMiss(benchmark::State& state)
{
    BM_BTreeLookupHelper(state, true);
}

static void
BM_BTreeLookupHit(benchmark::State& state)
{
    BM_BTreeLookupHelper(state, false);
}

BENCHMARK(BM_BTreeLookupMiss)->Arg(10)->Arg(1000)->Arg(100000)->UseRealTime();

BENCHMARK(BM_BTreeLookupHit)->Arg(10)->Arg(1000)->Arg(100000)->UseRealTime();
...
BENCHMARK(BM_BTreeInsert)->Arg(1000)->Arg(10000)->Arg(100000)->UseRealTime();
