...
protected:
    void SetUp() override
    {
        ...
        // construct a schema for testing that includes inline data, variable data, bit-data and
        // nullable fields
        std::vector<SchemaColumn> columns({{"variable", 0, SchemaType::TEXT, 0, false},
    ...
    void _verify(const Extent::Row &row,
                 const std::string &variable,
                 uint64_t fixed,
                 bool bit,
                 int32_t nullable,
    ...
    ExtentPtr _create_and_populate(bool add_empty_variable = false)
    {
        // create an extent
        ExtentPtr extent = std::make_shared<Extent>(ExtentType{false}, 0, _schema->row_size());

        // fill it with data
        _populate(extent->append(), "duplicate", 321837248973189LL, true, 15, false);
        _populate(extent->append(), "duplicate", 15, false, 0, true);
        _populate(extent->append(), "different", 4372895, true, 12, false);

        if (add_empty_variable) {
            _populate(extent->append(), "", 4372896, true, 15, false);
        }
...
TEST_F(Extent_Test, WriteAndReadViaDisk)
{
    ...
    auto handle = IOMgr::get_instance()->open("/tmp/test_extent_file", IOMgr::IO_MODE::WRITE, true);
...
TEST_F(Extent_Test, InsertAndRemove)
{
    ...
    _populate(extent->insert(i), "different", 6666, false, 7777, false);
...
TEST_F(Extent_Test, WriteAndSplit)
{
    ...
    ASSERT_EQ(extent->row_count(), pair.first->row_count() + pair.second->row_count());
