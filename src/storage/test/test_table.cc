...
void
PrintTo(const CacheSize &cacheSize, std::ostream *os)
{
...
protected:
    void SetUp() override
    {
        ...
        std::string overrides = std::format(
            "storage.data_cache_size={};storage.page_cache_size={};storage.btree_cache_size={};"
            "storage.max_extent_per_page={}",
            sizes.data_cache_size, sizes.page_cache_size, sizes.btree_cache_size,
            sizes.max_extent_per_page);
    ...
    TablePtr _create_table(uint64_t table_id, uint64_t xid, const std::vector<TableRoot> &roots)
    {
        ...
        return std::make_shared<Table>(_db_id, table_id, xid, _base_dir, _primary_keys, keys,
                                       tbl_meta, _schema);
    }

    MutableTablePtr _create_mtable(uint64_t table_id,
                                   uint64_t xid,
                                   const std::vector<TableRoot> &roots)
    {
        TableMetadata tbl_meta;
        tbl_meta.roots = roots;
        tbl_meta.snapshot_xid = 1;
        ...
        return std::make_shared<MutableTable>(_db_id, table_id, xid - 1, xid, _base_dir,
                                              _primary_keys, keys, tbl_meta, _schema);
    }

    std::shared_ptr<Tuple> _create_key(const std::string &name)
    {
    ...
    std::shared_ptr<Tuple> _create_value(uint64_t table_id,
    ...
    void _populate_table(MutableTablePtr mtable, uint64_t xid)
    {
        csv::CSVReader reader("test_btree_simple.csv");
        for (auto &&r : reader) {
            // insert data to the tree
            mtable->insert(std::make_shared<FieldTuple>(_csv_fields, r), xid,
    ...
    public:
        ...
        Request(MutableTablePtr table,
            ...
            : _table(table), _xid(xid), _extent_id(extent_id), _tuples(tuples)
        {
        }
...
TEST_P(Table_Test, CreateEmpty)
{
    ...
    sys_tbl_mgr::Client::get_instance()->update_roots(mtable->db(), mtable->id(), target_xid,
                                                      metadata);
...
TEST_P(Table_Test, SingleXactMutations)
{
    ...
    std::vector<std::string> keys = {"cibeson0", "pnisard0", "unardi0", "asineath0", "sfrankland0"};
    for (auto &key : keys) {
    ...
    std::vector<TuplePtr> update_values = {
        _create_value(6, "ctipton5", 100), _create_value(6, "callsepp5", 100),
        _create_value(6, "mpinwell5", 100), _create_value(6, "oblackborn5", 100),
    ...
    std::vector<TuplePtr> upsert_values = {
        _create_value(1500, "cibeson0", 1), _create_value(1500, "pnisard0", 1),
        _create_value(1500, "unardi0", 1), _create_value(1500, "asineath0", 1),
    ...
    upsert_values = {_create_value(3, "tcases2", 103), _create_value(3, "lharback2", 103),
                     _create_value(3, "ehalpeine2", 103), _create_value(3, "astenner2", 103),
...
TEST_P(Table_Test, MultiXactMutations)
{
    ...
    keys = {"tdockrell1", "gatwater1", "llandon1", "nmingardi1", "lstrapp1"};
    for (auto &key : keys) {
    ...
    update_values = {_create_value(7, "dgrgic6", 101), _create_value(7, "nbeaglehole6", 101),
                     _create_value(7, "trosendorf6", 101), _create_value(7, "ldana6", 101),
                     _create_value(7, "gridulfo6", 101)};
    std::vector<std::string> update_keys = {"dgrgic6", "nbeaglehole6", "trosendorf6", "ldana6",
                                            "gridulfo6"};
    ...
    upsert_values = {_create_value(1501, "tdockrell1", 2), _create_value(1501, "gatwater1", 2),
                     _create_value(1501, "llandon1", 2), _create_value(1501, "nmingardi1", 2),
                     _create_value(1501, "lstrapp1", 2)};
    std::vector<std::string> upsert_keys = {"tdockrell1", "gatwater1", "llandon1", "nmingardi1",
                                            "lstrapp1"};
    ...
    upsert_values = {_create_value(4, "pblythin3", 104), _create_value(4, "kradki3", 104),
                     _create_value(4, "sdeery3", 104), _create_value(4, "nhessentaler3", 104),
                     _create_value(4, "lhiskey3", 104)};
    upsert_keys = {"pblythin3", "kradki3", "sdeery3", "nhessentaler3", "lhiskey3"};
    for (int i = 0; i < upsert_keys.size(); i++) {
        auto &&search_key = _create_key(upsert_keys[i]);
        uint64_t extent_id = table->primary_lookup(search_key);
        mtable->upsert(upsert_values[i], target_xid, extent_id);
...
TEST_P(Table_Test, MultiThreadMutations)
{
    ...
    for (auto &entry : tuple_map) {
        auto request =
            std::make_shared<Request>(mtable, target_xid, entry.first, std::move(entry.second));
