...
protected:
    void SetUp() override
    {
        ...
        std::vector<SchemaColumn> columns({{"table_id", 0, SchemaType::UINT64, 0, false, 1},
    ...
    std::shared_ptr<MutableBTree> _create_mutable_btree(const std::filesystem::path &name,
                                                        uint64_t xid)
    {
    ...
    std::shared_ptr<MutableBTree> _get_mutable_btree(const std::filesystem::path &name,
                                                     uint64_t xid,
                                                     uint64_t extent_id)
    {
        // construct a mutable b-tree for inserting data
    ...
    std::shared_ptr<Tuple> _create_key(const std::string &name, uint64_t table_id, uint16_t idx)
    {
    ...
    TuplePtr _gen_tuple(csv::CSVRow &row, uint16_t idx)
    {
    ...
    void _populate_btree(MutableBTreePtr btree, uint16_t idx)
    {
        // pull data to insert
    ...
    void _verify_names(BTreePtr tree, int target)
    {
        int count = 0;
    ...
    void _verify_unique_names(BTreePtr tree, int target)
    {
        int count = 0;
    ...
    public:
        ...
        Request(std::shared_ptr<MutableBTree> btree, const Type &type, TuplePtr tuple)
            : _btree(btree), _type(type), _tuple(tuple)
        {
        }
...
TEST_P(BTree_Test, Insert10)
{
    ...
    // now read the tree back and make sure there are the right number of entries and that they are
    // in-order
...
TEST_P(BTree_Test, ThreadedInsertAndRemove)
{
    ...
    tester.set_verify([this, btree2]() {
        ...
        auto tree =
            std::make_shared<BTree>(_base_dir / "ThreadedInsertAndRemove", 2, _schema, offset);
