...
namespace {
    ...
    TEST_P(BTree_Test, Insert10) {
        ...
        auto tree = std::make_shared<BTree>(0, _base_dir / "Insert10", xid, _schema, offset, constant::MAX_EXTENT_SIZE);
    ...
    TEST_P(BTree_Test, InsertAll) {
        ...
        // now read the tree back and make sure there are the right number of entries and that they are in-order
        auto tree = std::make_shared<BTree>(0, _base_dir / "InsertAll", 1, _schema, offset, constant::MAX_EXTENT_SIZE);
    ...
    TEST_P(BTree_Test, Search) {
        ...
        auto tree = std::make_shared<BTree>(0, _base_dir / "Search", 1, _schema, offset, constant::MAX_EXTENT_SIZE);
    ...
    TEST_P(BTree_Test, InsertAndRemove) {
        ...
        auto tree = std::make_shared<BTree>(0, _base_dir / "InsertAndRemove", 1, _schema, offset_1, constant::MAX_EXTENT_SIZE);
        _verify_names(tree, 5000);
        ...
        // check XID 1 for all entries
        tree = std::make_shared<BTree>(0, _base_dir / "InsertAndRemove", 1, _schema, offset_1, constant::MAX_EXTENT_SIZE);
        _verify_names(tree, 5000);

        // check XID 2 for half the entries
        tree = std::make_shared<BTree>(0, _base_dir / "InsertAndRemove", 2, _schema, offset_2, constant::MAX_EXTENT_SIZE);
    ...
    TEST_P(BTree_Test, InsertAndRemoveAll) {
        ...
        // check XID 1 for all entries
        auto tree = std::make_shared<BTree>(0, _base_dir / "InsertAndRemoveAll", 1, _schema, offset_1, constant::MAX_EXTENT_SIZE);
        _verify_names(tree, 5000);
        ...
        // check XID 1 for all entries
        tree = std::make_shared<BTree>(0, _base_dir / "InsertAndRemoveAll", 1, _schema, offset_1, constant::MAX_EXTENT_SIZE);
        _verify_names(tree, 5000);

        // check XID 2 for no entries
        tree = std::make_shared<BTree>(0, _base_dir / "InsertAndRemoveAll", 2, _schema, offset_2, constant::MAX_EXTENT_SIZE);
    ...
    TEST_P(BTree_Test, InsertMany) {
        ...
        auto tree = std::make_shared<BTree>(0, _base_dir / "InsertMany", 1, _schema, offset, constant::MAX_EXTENT_SIZE);
    ...
    TEST_P(BTree_Test, ThreadedInserts) {
        ...
        tester.set_verify([this, btree]() {
            ...
            auto tree = std::make_shared<BTree>(0, _base_dir / "ThreadedInserts", 1, _schema, offset, constant::MAX_EXTENT_SIZE);
    ...
    TEST_P(BTree_Test, ThreadedInsertAndRemove) {
        ...
        // get a mutable btree to perform inserts
        auto btree1 = std::make_shared<MutableBTree>(0, filename, _keys, _schema, 1, constant::MAX_EXTENT_SIZE);
        btree1->init_empty();

        auto btree2 = std::make_shared<MutableBTree>(0, filename, _keys, _schema, 2, constant::MAX_EXTENT_SIZE);
        ...
        tester.set_verify([this, filename, btree1, btree2]() {
            ...
            auto tree = std::make_shared<BTree>(0, filename, 1, _schema, offset, constant::MAX_EXTENT_SIZE);
        ...
        tester.set_verify([this, btree2]() {
            ...
            auto tree = std::make_shared<BTree>(0, _base_dir / "ThreadedInsertAndRemove", 2, _schema, offset, constant::MAX_EXTENT_SIZE);
    ...
    TEST_P(BTree_Test, ThreadedInsertsTwoFiles) {
        ...
        tester.set_verify([this, btree1, btree2]() {
            ...
            auto tree = std::make_shared<BTree>(0, _base_dir / "ThreadedInsertsOne", 1, _schema, offset1, constant::MAX_EXTENT_SIZE);

            // check for all entries
            _verify_unique_names(tree, 50000);

            // check the second file
            tree = std::make_shared<BTree>(0, _base_dir / "ThreadedInsertsTwo", 1, _schema, offset2, constant::MAX_EXTENT_SIZE);
