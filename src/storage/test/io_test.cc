 ...
 */
static bool
compare_data(std::shared_ptr<std::vector<char>> data1, std::shared_ptr<std::vector<char>> data2)
{
            ...
            int count = 1)
{
    ...
    std::shared_ptr<springtail::IOResponseRead> read_response =
        fh_read->read(write_response->offset);
...
{
    std::cout << "\nTesting Sync Write: len=" << len << ", offset=" << offset << ", count=" << count
              << std::endl;
    ...
    std::cout << "  - offset=" << write_response->offset
              << ", next_offset=" << write_response->next_offset << std::endl;
...
TEST(IOTest, FHTests)
{
    ...
    for (int i = 0; i < files; i++) {
        ...
        for (int j = 0; j < count; j++) {
            std::shared_ptr<springtail::IOHandle> fh_write =
                IOMgr->open(path, springtail::IOMgr::IO_MODE::WRITE, false);
            std::shared_ptr<springtail::IOHandle> fh_read =
                IOMgr->open(path, springtail::IOMgr::IO_MODE::READ, false);
...
TEST(IOTest, IOTests)
{
    ...
    std::shared_ptr<springtail::IOHandle> fh_append =
        IOMgr->open(FILE1, springtail::IOMgr::IO_MODE::APPEND, true);
    std::shared_ptr<springtail::IOHandle> fh_read =
        IOMgr->open(FILE1, springtail::IOMgr::IO_MODE::READ, true);
    ...
    std::shared_ptr<springtail::IOHandle> fh_write =
        IOMgr->open(FILE2, springtail::IOMgr::IO_MODE::WRITE, false);
