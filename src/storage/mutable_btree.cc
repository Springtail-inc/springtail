...
MutableBTree::MutableBTree(const std::filesystem::path &file,
                           ...
                           uint64_t xid)
    : _file(file), _sort_keys(keys), _xid(xid), _finalized(true)
{
                                  ...
                                  MutableFieldPtr offset_f)
{
    ...
    for (auto &&i = new_pages.rbegin(); i != new_pages.rend(); i++) {
        ...
        SPDLOG_DEBUG_MODULE(LOG_BTREE, "Adding branch entry to child extent_id: {}",
                            page->extent_id);
        ...
        auto kv =
            std::make_shared<KeyValueTuple>(page->prev_key->fields(), value, page->prev_key->row());
...
void
MutableBTree::Page::insert(TuplePtr search_key, TuplePtr value, MutableFieldArrayPtr fields)
{
...
MutableBTree::Page::flush(uint64_t xid)
{
    ...
    ExtentType type =
        (_cache_page->extent_count() > 1) ? ExtentType(this->type.is_branch()) : this->type;
...
void
MutableBTree::Page::set_cache_page(StoragePagePtr cache_page, ExtentSchemaPtr schema)
{
    // note: we must be holding the disk_mutex, so no need to lock the access mutex
                                 ...
                                 boost::unique_lock<boost::shared_mutex> &cache_lock)
{
    ...
    while (_cache->size > _cache->max_size) {
        ...
        // note: if the page is not dirty, was already flushed, or has never been accessed since
        // being
        ...
        // scoped for locks
        {
            ...
            // note: should never block since we removed the entry from the cache while it wasn't in
            // use
...
MutableBTree::_read_page_internal(PagePtr page)
{
    ...
    ExtentSchemaPtr schema =
        (cache_page->header().type.is_branch()) ? _branch_schema : _leaf_schema;
...
MutableBTree::_find_leaf(TuplePtr key)
{
    ...
    while (node->page->type.is_branch()) {
        ...
        // if the child was flushed while we were reading the page, we need to re-find it in the
        // parent
        if (!child) {
            continue;
        }
...
MutableBTree::_flush_page_internal(PagePtr page, PagePtr parent)
{
...
void
MutableBTree::_flush_page(PagePtr page, PagePtr parent)
{
...
MutableBTree::_flush_root(PagePtr page)
{
    ...
    std::vector<PagePtr> &&new_pages =
        (page->empty()) ? page->flush_empty_root(_xid) : page->flush(_xid);
    ...
    if (new_pages.size() > 1) {
        ...
        for (PagePtr child : new_pages) {
            ...
            SPDLOG_DEBUG_MODULE(LOG_BTREE, "Adding root entry to child extent_id: {}",
                                child->extent_id);
        ...
        new_root = std::make_shared<Page>(this, extent_id, value_key, std::move(cache_page),
                                          _branch_schema);
...
void
MutableBTree::_remove_page_internal(PagePtr page, PagePtr parent)
{
...
void
MutableBTree::_remove_page(PagePtr page, PagePtr parent)
{
...
void
MutableBTree::_init_schemas(ExtentSchemaPtr schema, const std::vector<std::string> &keys)
{
    _leaf_schema = schema;
