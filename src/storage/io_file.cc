...
IOSysFH::IOSysFH(const std::filesystem::path &path, const IOMgr::IO_MODE &mode, bool is_compressed)
    : _path(path), _fd(-1), _is_compressed(is_compressed), _is_dirty(false), is_busy(false)
{
    int fmode;
    if (mode == IOMgr::IO_MODE::READ) {
        _is_readonly = true;
 ...
 */
void
IOSysFH::read(IORequestRead *const request, std::shared_ptr<Decompressor> decompressor)
{
    ...
    if (hdr[0] == HDR_MAGIC_COMPRESSED[0] && hdr[1] == HDR_MAGIC_COMPRESSED[1] &&
        hdr[2] == HDR_MAGIC_COMPRESSED[2]) {
        decode_error = false;
        is_compressed = true;
    } else if (hdr[0] == HDR_MAGIC_UNCOMPRESSED[0] && hdr[1] == HDR_MAGIC_UNCOMPRESSED[1] &&
               hdr[2] == HDR_MAGIC_UNCOMPRESSED[2]) {
        decode_error = false;
        is_compressed = false;
    }
    ...
    SPDLOG_DEBUG_MODULE(LOG_STORAGE, "IOSysFH::read bytes read={}, hdr_off={}", bytes_read,
                        hdr_off);
                         ...
                         bool is_compressed)
{
    ...
    if (is_compressed) {
        ...
        try {
            ...
            for (int i = 0; i < count; i++) {
                ...
                SPDLOG_DEBUG_MODULE(LOG_STORAGE, "IOSysFH::_internal_write: compressing vector: {}",
                                    i);
        ...
        if (size <= compressed_size) {
            ...
            SPDLOG_DEBUG_MODULE(
                LOG_STORAGE,
                "IOSys::internal_write: Not compressing data, compressed size too big: {} vs {}",
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (is_compressed) {
            ...
            SPDLOG_DEBUG_MODULE(LOG_STORAGE,
                                "IOSysFH::internal_write (compressed); idx={}, size={}, csize={}",
                                i, size, csize);
        } else {
            ...
            SPDLOG_DEBUG_MODULE(LOG_STORAGE,
                                "IOSysFH::internal_write (uncompressed); idx={}, size={}", i, size);
        }
...
void
IOSysFH::append(IORequestAppend *const request, std::shared_ptr<Compressor> compressor)
{
    // default error response
...
void
IOSysFH::sync(IORequestSync *const request)
{
