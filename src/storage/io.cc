...
{
    std::shared_ptr<IORequestRead> req =
        std::make_shared<IORequestRead>(_path, _is_compressed, pos, callback);
...
IOHandle::async_append(const char *buffer, int length, io_append_callback_fn callback) const
{
    std::shared_ptr<std::vector<char>> data =
        std::make_shared<std::vector<char>>(buffer, buffer + length);
    return async_append(data, callback);
}

std::future<std::shared_ptr<IOResponseAppend>>
IOHandle::async_append(std::shared_ptr<std::vector<char>> data,
                       io_append_callback_fn callback) const
{
    std::shared_ptr<IORequestAppend> req =
        std::make_shared<IORequestAppend>(_path, _is_compressed, data, callback);
...
std::future<std::shared_ptr<IOResponseAppend>>
IOHandle::async_append(const std::vector<std::shared_ptr<std::vector<char>>> &data,
                       io_append_callback_fn callback) const
{
    std::shared_ptr<IORequestAppend> req =
        std::make_shared<IORequestAppend>(_path, _is_compressed, data, callback);
...
IOHandle::async_write(uint64_t offset,
                      std::shared_ptr<std::vector<char>> data,
                      io_write_callback_fn callback) const
{
    if (_is_compressed == true) {
        throw StorageError();
    }
    std::shared_ptr<IORequestWrite> req =
        std::make_shared<IORequestWrite>(_path, _is_compressed, offset, data, callback);
...
{
    std::shared_ptr<IORequestSync> req =
        std::make_shared<IORequestSync>(_path, _is_compressed, callback);
