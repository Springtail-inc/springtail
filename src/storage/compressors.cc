...
Lz4Compressor::Lz4Compressor() { _lz4_stream = LZ4_createStream(); }

Lz4Compressor::~Lz4Compressor() { LZ4_freeStream(_lz4_stream); }
...
Lz4Compressor::compress_raw(std::shared_ptr<std::vector<char>> src, std::vector<char> &dst)
{
    ...
    int32_t dst_size = LZ4_compress_fast_extState(_lz4_stream, src->data(), dst.data(), src->size(),
                                                  target_size, 1);
...
Lz4Compressor::compress_block(const std::vector<char> &src, std::vector<char> &dst)
{
    ...
    int32_t dst_size = LZ4_compress_fast_continue(_lz4_stream, src.data(), dst.data() + offset + 8,
...
Lz4Decompressor::Lz4Decompressor() { _lz4_stream = LZ4_createStreamDecode(); }

Lz4Decompressor::~Lz4Decompressor() { LZ4_freeStreamDecode(_lz4_stream); }
                                ...
                                int offset)
{
    ...
    int32_t size =
        LZ4_decompress_safe(src.data(), dst->data() + offset, src.size(), dst->size() - offset);
