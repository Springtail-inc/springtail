...
{
    SPDLOG_DEBUG_MODULE(LOG_CACHE, "GET file {} eid {} xid {} txid {}", file, extent_id, access_xid,
                        target_xid);
    ...
    if (target_xid == access_xid) {
        return {_page_cache.get(), _page_cache->get(file, extent_id, access_xid, target_xid),
                flush_cb};
    }
...
StorageCache::PagePtr
StorageCache::PageCache::get_empty(const std::filesystem::path &file, uint64_t xid)
{
...
{
    SPDLOG_DEBUG_MODULE(LOG_CACHE, "PUT file {} eid {} s_xid {} e_xid {}", page->_file,
                        page->_extent_id, page->_start_xid, page->_end_xid);
    ...
    if (page->_extent_id == constant::UNKNOWN_EXTENT) {
        // note: we cannot release a dirty page with no original extent ID back to the cache unless
...
StorageCache::PageCache::_try_evict(PagePtr page)
{
    ...
    SPDLOG_DEBUG_MODULE(LOG_CACHE, "Page evict file {} eid {} xid {}", page->key().first,
                        page->key().second, page->xid());
...
StorageCache::Page::Page(const std::filesystem::path &file, uint64_t xid)
    : _use_count(1),
      _is_dirty(false),
      _file(file),
...
StorageCache::Page::lower_bound(TuplePtr tuple, ExtentSchemaPtr schema)
{
    ...
    auto extent_i = std::lower_bound(
        _extents.begin(), _extents.end(), *tuple,
    ...
    auto row_i = std::ranges::lower_bound(
        **extent, *tuple, [](const Tuple &lhs, const Tuple &rhs) { return lhs.less_than(rhs); },
        [&schema](const Extent::Row &row) { return FieldTuple(schema->get_sort_fields(), row); });

    // note: shouldn't be possible to hit end() given the above lower_bound() check to find the
    // extent
...
StorageCache::Page::upper_bound(TuplePtr tuple, ExtentSchemaPtr schema)
{
    ...
    //       SafeExtent to go out of scope before it is used in the comparison
    auto extent_i =
        std::upper_bound(_extents.begin(), _extents.end(), *tuple,
    ...
    auto row_i = std::ranges::upper_bound(
        **extent, *tuple, [](const Tuple &lhs, const Tuple &rhs) { return lhs.less_than(rhs); },
        [&schema](const Extent::Row &row) { return FieldTuple(schema->get_sort_fields(), row); });

    // note: shouldn't be possible to hit end() given the above upper_bound() check to find the
    // extent
    CHECK(row_i != (*extent)->end());
...
void
StorageCache::Page::insert(TuplePtr tuple, ExtentSchemaPtr schema)
{
    boost::unique_lock lock(_mutex);
    _is_dirty = true;
    ...
    auto extent_i = std::lower_bound(
        _extents.begin(), _extents.end(), *key,
        [this, &schema](const ExtentRef &ref, const Tuple &key) {
            auto extent = ref.make_safe_extent(_file);
            return FieldTuple(schema->get_sort_fields(), (*extent)->back()).less_than(key);
        });
    ...
    auto row_i = std::ranges::lower_bound(
        **extent, *key, [](const Tuple &lhs, const Tuple &rhs) { return lhs.less_than(rhs); },
        [&schema](const Extent::Row &row) { return FieldTuple(schema->get_sort_fields(), row); });

    // note: row's key should *not* match the tuple's key
    CHECK(row_i == (*extent)->end() || !FieldTuple(schema->get_sort_fields(), *row_i).equal(*key));
...
void
StorageCache::Page::append(TuplePtr tuple, ExtentSchemaPtr schema)
{
    boost::unique_lock lock(_mutex);
    _is_dirty = true;
...
bool
StorageCache::Page::upsert(TuplePtr tuple, ExtentSchemaPtr schema)
{
    boost::unique_lock lock(_mutex);
    _is_dirty = true;
...
void
StorageCache::Page::update(TuplePtr tuple, ExtentSchemaPtr schema)
{
    boost::unique_lock lock(_mutex);
    _is_dirty = true;
...
void
StorageCache::Page::remove(TuplePtr key, ExtentSchemaPtr schema)
{
    boost::unique_lock lock(_mutex);
    _is_dirty = true;
                            ...
                            uint64_t target_xid)
{
    ...
    for (auto &ref : _extents) {
        ...
        SPDLOG_DEBUG_MODULE(LOG_CACHE, "{}@{} (size: {}) to {}@{} (size: {})",
                            (*old_extent)->extent_id(), (*old_extent)->header().xid,
                            (*old_extent)->header().row_size, new_extent->extent_id(),
                            new_extent->header().xid, new_extent->header().row_size);
                                 ...
                                 ExtentSchemaPtr schema)
{
    ...
    // insert the two new extents; insert() occurs before the provided iterator, so inserted in
    // reverse order
    ...
    for (auto &ref : _extents) {
        ...
        if (!ref.is_clean()) {
            ...
            if ((*e)->state() == CacheExtent::State::DIRTY) {
                ...
                // XXX should do these asynchronously to get better parallelism when there are
                // multiple extents
            ...
            ref = e.get_ref();
            SPDLOG_DEBUG_MODULE(LOG_CACHE, "Flushing extent {} -- new extent {}", _extent_id,
                                ref.id());
        }
...
StorageCache::CacheExtentPtr
StorageCache::DataCache::_get(uint64_t cache_id, bool mark_dirty)
{
...
StorageCache::CacheExtentPtr
StorageCache::DataCache::get_empty(const std::filesystem::path &file, const ExtentHeader &header)
{
    boost::unique_lock lock(_mutex);
...
StorageCache::CacheExtentPtr
StorageCache::DataCache::_use_direct(const CacheExtentPtr &extent, bool mark_dirty)
{
    // if the extent is being flushed, must block until complete
    if (extent->_state == CacheExtent::State::FLUSHING) {
        // mark ourselves as a user of the extent to prevent eviction post-flush()
    ...
    // if we aren't asking for a dirty extent, or it's already dirty / mutable, we can return
    // immediately
    if (!mark_dirty || extent->_state == CacheExtent::State::DIRTY ||
        extent->_state == CacheExtent::State::MUTABLE) {
        if (mark_dirty) {
            extent->_state = CacheExtent::State::DIRTY;
        }
