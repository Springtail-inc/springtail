...
static constexpr char USER_SELECT[] =
    "select username, password, databases from public.springtail_get_user_access()";

static const std::vector<std::string> AUTH_TYPE_TO_STR = {"TRUST", "MD5", "SCRAM"};

User::User(const std::string &username, const std::string &password) : _username(username), _salt(0)
{
...
void
User::set_password(const std::string &password)
{
    ...
    while (!_is_shutting_down()) {
        while (!conn.is_connected()) {
            try {
                ...
                SPDLOG_ERROR("Failed to connect to primary database; will retry in {} seconds",
                             _sleep_interval);
        ...
        try {
            ...
            SPDLOG_ERROR(fmt::format("Error: {}. Failed to excute the query; will try to reconnect",
                                     e.what()));
            conn.disconnect();
            sleep(_sleep_interval);
            continue;
