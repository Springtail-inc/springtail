...
protected:
    ...
    static void TearDownTestSuite() { DatabaseMgr::shutdown(); }
    ...
    void _add_database(uint64_t db_id, const std::string &name)
    {
        ...
        // hset db_config:1234 2 "{\"name\": \"springtail_1\", \"replication_slot\":
        // \"springtail_slot\", \"publication_name\": \"springtail_pub\", \"include\": {\"schemas\":
        // [\"*\"]}}"
        ...
        std::string config_str = "{" + name_str + ", " + replication_slot + ", " +
                                 publication_name + ", \"include\": {\"schemas\": [\"*\"]}}";
    ...
    void _add_table(uint64_t db_id,
                    ...
                    bool test = true)
    {
        if (test) {
            EXPECT_FALSE(
                DatabaseMgr::get_instance()->is_table_replicated(db_id, "public", schema, table));
        }
    ...
    void _remove_table(uint64_t db_id,
                       const std::string &schema,
                       const std::string &table,
                       bool test = true)
    {
