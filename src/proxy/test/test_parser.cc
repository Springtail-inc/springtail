...
TEST(ProxyParser_Test, TestParser)
{
    ...
    for (int i = 0; i < tests.size(); i++) {
        ...
        std::string name;
        for (auto &r : res) {
            LOG_INFO("Query: {} is {}", std::get<0>(tests[i]), (r->is_read_safe ? "readable" : "NOT readable"));
            if (!r->is_read_safe) {
                is_readable = false;
            }
            if (!r->name.empty()) {
        ...
        if (is_readable != expected_readable) {
            LOG_ERROR("Mismatch in query expectations: expected={}, got={}", expected_readable, is_readable);
            Parser::dump_parse_tree(std::get<0>(tests[i]));
        }
        if (name != std::get<2>(tests[i])) {
            LOG_INFO("Name mismatch: {} != {}", name, std::get<2>(tests[i]));
        }
