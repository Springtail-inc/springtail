    ...
    while (db_it != _free_sessions.end()) {
        ...
        list.remove_if([instance](const ServerSessionWeakPtr &s) {
            ...
            if ((session != nullptr) && (session->get_instance() == instance)) {
                PROXY_DEBUG(LOG_LEVEL_DEBUG2,
                            "Evicting session from pool: [S:{:d}], db_id: {}, username: {}",
                            session->id(), session->database_id(), session->username());
                PROXY_DEBUG(LOG_LEVEL_DEBUG2, "Instance: {}", session->get_instance()->to_string());
            } else {
                PROXY_DEBUG(LOG_LEVEL_DEBUG2, "Session not evicted from pool: [S:{:d}]",
                            session->id());
                PROXY_DEBUG(LOG_LEVEL_DEBUG2, "Instance: {}", session->get_instance()->to_string());
            }
...
void
DatabaseSet::_release_session(ServerSessionPtr session, int num_instances, bool deallocate)
{
                                   ...
                                   const std::unordered_map<std::string, std::string> &parameters)
{
    ...
    return ServerSession::create(server, user, db_name.value(), prefix(), shared_from_this(), _type,
                                 parameters);
}
...
void
Database::add_schema_tables(
    const std::vector<std::pair<std::string, std::string>> &schema_table_pairs)
{
...
DatabaseMgr::init()
{
    ...
    set_primary(primary_instance_id,
                std::make_shared<DatabaseInstance>(Session::Type::PRIMARY, host, "", port));
    ...
    for (const auto &fdw_id : fdw_id_list) {
        ...
        if (host.has_value() && port.has_value()) {
            ...
            add_replica(std::make_shared<DatabaseInstance>(Session::Type::REPLICA, host.value(),
                                                           db_prefix.value(), port.value()));
        } else {
            SPDLOG_ERROR("Could not find the value for replica database {} either host or port",
                         fdw_id);
    ...
    _config_sub_thread.add_subscriber(
        db_change_channel, [this]() { this->_init_replicated_dbs_subscriber(); },
    ...
    _config_sub_thread.add_subscriber(
        state_change_channel, [this]() { this->_init_db_states_subscriber(); },
        [this](const std::string &msg) { _handle_db_state_change(msg); });

    std::string db_table_change_channel =
        fmt::format(redis::PUBSUB_DB_TABLE_CHANGES, _db_instance_id);
    _data_sub_thread.add_subscriber(
        db_table_change_channel, [this]() { this->_init_db_tables_subscriber(); },
        [this](const std::string &msg) { _handle_db_table_change(msg); });
...
void
DatabaseMgr::_init_db_states_subscriber()
{
...
void
DatabaseMgr::_handle_db_table_change(const std::string &msg)
{
    ...
    if (action == "add") {
        ...
        SPDLOG_DEBUG_MODULE(LOG_PROXY, "Added schema: {}, table: {} to database {}", schema, table,
                            db_id);
    } else if (action == "remove") {
        iter->second->remove_schema_table(schema, table);
        SPDLOG_DEBUG_MODULE(LOG_PROXY, "Removed schema: {}, table: {} from database {}", schema,
                            table, db_id);
    } else {
...
DatabaseMgr::_handle_replicated_dbs_change(const std::string &msg)
{
    ...
    switch (action) {
        ...
        case redis::db_state_change::DB_ACTION_REMOVE:
            ...
            break;
        default:
            SPDLOG_DEBUG_MODULE(LOG_PROXY, "Unsupported action: {}",
