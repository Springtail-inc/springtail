...
/** reset query string; similar to rollback, discard all, set search path, but discard all can't be
 * run with other statements */
constexpr char RESET_QUERY[] =
    "ROLLBACK;CLOSE ALL;SET SESSION AUTHORIZATION DEFAULT;RESET ALL;DEALLOCATE ALL;UNLISTEN "
    "*;SELECT pg_advisory_unlock_all();DISCARD PLANS;DISCARD TEMP;DISCARD SEQUENCES;SET "
    "SEARCH_PATH TO DEFAULT";
...
{
    _state = STARTUP;
    PROXY_DEBUG(LOG_LEVEL_DEBUG1, "[S:{}] Server connected: endpoint={}", _id,
                connection->endpoint());
}
    ...
    for (const auto &param : parameters) {
        ...
        if (it == _parameters.end() || it->second != param.second) {
            PROXY_DEBUG(LOG_LEVEL_DEBUG3,
                        "[S:{}] Server startup parameter mismatch: {}={}; expected={}", _id,
                        param.first, it->second, param.second);
            return false;
        }
    }
...
{
    PROXY_DEBUG(LOG_LEVEL_DEBUG1, "[S:{}] Server session processing packet: state={:d}", _id,
                (int8_t)_state);
...
ServerSession::_handle_reset_session_message()
{
    ...
    PROXY_DEBUG(LOG_LEVEL_DEBUG1, "[S:{}] Server session message: code={}, length={}", _id, code,
                msg_length);
    if (msg_length == 0) {
        // no message to read
        return;
    ...
    switch (code) {
        case 'Z': {  // Ready for query
            ...
            if (status == 'E') {
                ...
                SPDLOG_WARN("[S:{}] Error in transaction while resetting session", _id);
                _state = ERROR;
                return;
            }
            ...
            if (_state == RESET_SESSION_PARAMS) {
                ...
                SessionMsgPtr msg =
                    std::make_shared<SessionMsg>(SessionMsg::MSG_SERVER_CLIENT_AUTH_DONE);
                queue_msg(msg);
                return;
            }
        ...
        default:
            ...
            PROXY_DEBUG(LOG_LEVEL_DEBUG3,
                        "[S:{}] Reset session, dropping message: code={}, length={}", _id, code,
                        msg_length);
            break;
    }
}
...
ServerSession::_handle_message_from_server()
{
    ...
    PROXY_DEBUG(LOG_LEVEL_DEBUG1, "[S:{}] Server session message: code={}, length={}, state={}",
    ...
    switch (code) {
        ...
        case 't':  // Parameter description (describe)
        {
            ...
            BufferPtr buffer = _read_message(code, msg_length);
            queue_msg(std::make_shared<SessionMsg>(SessionMsg::MSG_SERVER_CLIENT_FORWARD, buffer,
                                                   _seq_id));
        ...
        case 'G': {
            ...
            BufferPtr buffer = _read_message(code, msg_length);
            SessionMsgPtr msg = std::make_shared<SessionMsg>(
                SessionMsg::MSG_SERVER_CLIENT_COPY_READY, buffer, _seq_id);
            queue_msg(msg);
            return;
        }
    ...
    switch (code) {
        ...
        case 'S': {
            ...
            if (_state <= AUTH_DONE) {
                // XXX still in auth negotiation state, if primary we should save these for the
                // client
                break;
            }
        ...
        case 'E':
            ...
            if (_state == QUERY) {
                ...
                // forward to client
                SessionMsgPtr msg = std::make_shared<SessionMsg>(
                    SessionMsg::MSG_SERVER_CLIENT_FORWARD, buffer, _seq_id);
                queue_msg(msg);
            } else {
        ...
        case 'Z': {
            ...
            // forward to client
            SessionMsgPtr msg = std::make_shared<SessionMsg>(SessionMsg::MSG_SERVER_CLIENT_FORWARD,
                                                             buffer, _seq_id);
            queue_msg(msg);
...
void
ServerSession::_send_startup_msg(uint64_t seq_id)
{
...
ServerSession::_handle_auth_scram_continue(BufferPtr buffer)
{
    ...
    if (!read_server_first_message(&_login->scram_state, input, &_login->scram_state.server_nonce,
                                   &_login->scram_state.salt, &salt_len,
...
ServerSession::_decode_error_buffer(BufferPtr buffer, uint64_t seq_id)
{
    ...
    SPDLOG_ERROR("Error response from server: seq_id: {}, {}, {}, {}", seq_id, text, code, message);
...
ServerSession::_handle_dependency_response(bool error)
{
    ...
    PROXY_DEBUG(LOG_LEVEL_DEBUG1, "[S:{}] Query dependency complete, count: {:d}/{:d}", _id,
                query_status->dependency_complete_count, query_status->dependency_count);
...
ServerSession::_handle_ready_for_query_response(char xact_status)
{
    ...
    if (query_status != nullptr && query_status->msg->data()->type == QueryStmt::Type::SYNC) {
        ...
        SessionMsgPtr msg =
            std::make_shared<SessionMsg>(SessionMsg::MSG_SERVER_CLIENT_READY, msg_status);
        queue_msg(msg);
    }
...
ServerSession::_handle_query_response()
{
    ...
    PROXY_DEBUG(LOG_LEVEL_DEBUG1, "[S:{}] Query complete, count: {:d}/{:d}, query_stmt: {}", _id,
    ...
    PROXY_DEBUG(LOG_LEVEL_DEBUG1, "[S:{}] All queries complete for this msg, seq_id: {}", _id,
...
ServerSession::_handle_msg_to_server(SessionMsgPtr msg)
{
    ...
    PROXY_DEBUG(LOG_LEVEL_DEBUG1,
                "[S:{}] Server session: msg to server, msg: {}, seq_id: {}, query_count: {}", _id,
                msg->type_str(), msg->seq_id(), query_status->query_count);
                      ...
                      const std::unordered_map<std::string, std::string> &params)
{
    ...
    ServerSessionPtr session = std::make_shared<ServerSession>(connection, server, user, database,
                                                               prefix, instance, params, type);
    PROXY_DEBUG(LOG_LEVEL_DEBUG1, "[S:{}] Created connection for server session, to: db={}",
                session->id(), database);
