...
ClientAuthorization::process_auth_data(uint64_t seq_id)
{
    switch (_state) {
        ...
        default:
            // do nothing
            LOG_DEBUG(LOG_PROXY, LOG_LEVEL_DEBUG2, "[C:{}] Client auth invalid state: {}", _id, state_to_string(_state, authorization_state_names));
            break;
    }
...
void
ClientAuthorization::_handle_startup(uint64_t seq_id)
{
...
ClientAuthorization::_send_auth_req(uint64_t seq_id)
{
    ...
    // log the buffer
    Session::log_buffer(Session::Type::CLIENT, _id, false, '\0', buffer->size(), buffer->data(),
                        seq_id);
}
        ...
        switch (_login->type) {
            case MD5: {
                ...
                if (client_passwd.empty() || client_passwd.size() != MD5_PASSWD_LEN) {
                    LOG_ERROR("Empty password received; or password length mismatch");
                    throw ProxyAuthError();
                }
                ...
                if (!pg_md5_encrypt(_login->password.c_str() + 3,
                    ...
                    LOG_ERROR("Failed to calculate MD5 hash");
                    throw ProxyAuthError();
                }
                md5[MD5_PASSWD_LEN] = '\0';

                if (strcmp(md5, client_passwd.data()) != 0) {
                    LOG_ERROR("MD5 password mismatch: : {} <> {}", md5, client_passwd);
                    char data[128];
                    ...
                    // log buffer
                    Session::log_buffer(Session::Type::CLIENT, _id, false, '\0',
                                        write_buffer->size(), write_buffer->data(), seq_id);
            ...
            case SCRAM: {
                ...
                if (_login->scram_state.server_nonce == nullptr) {
                    ...
                    if (scram_type != "SCRAM-SHA-256") {
                        LOG_ERROR("Unsupported scram type: {}", scram_type);
                        throw ProxyAuthError();
                    }
            ...
            default:
                LOG_ERROR("Invalid login type: {}", static_cast<int8_t>(_login->type));
                throw ProxyAuthError();
        }
    }
}
...
void
ClientAuthorization::_encode_parameter_status(BufferPtr buffer,
...
ServerAuthorization::_handle_message(uint64_t seq_id)
{
    ...
    LOG_DEBUG(LOG_PROXY, LOG_LEVEL_DEBUG1,
                ...
                msg_length, state_to_string(_state, authorization_state_names));
...
ServerAuthorization::_handle_auth_md5(BufferPtr buffer, uint64_t seq_id)
{
    ...
    if (_login->type == PasswordType::MD5) {
        ...
        LOG_ERROR("Invalid password type for MD5");
        throw ProxyAuthError();
    }
...
ServerAuthorization::_handle_auth_scram(BufferPtr buffer, uint64_t seq_id)
{
    // get user login info
    if (_login == nullptr) {
        LOG_ERROR("Failed to get user login info");
        throw ProxyAuthError();
    }
    ...
    if (!found) {
        LOG_ERROR("No SASL mechanism found matching: SCRAM-SHA-256");
        throw ProxyAuthError();
    }
