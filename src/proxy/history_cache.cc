    ...
    for (auto it = _transaction_history._history.rbegin(); it != rend; ++it) {
        ...
        if (it->second->type == QueryStmt::SAVEPOINT && it->second->name == name) {
            ...
            _transaction_history._history.erase(std::next(it).base(),
                                                _transaction_history._history.end());
...
StatementCache::sync_transaction(char xact_status)
{
    ...
    PROXY_DEBUG(LOG_LEVEL_DEBUG3, "Syncing transaction: status: {}, in_error: {}", xact_status,
                _in_error);
...
StatementCache::commit_statement(QueryStmtPtr stmt, int completed, bool success)
{
    PROXY_DEBUG(LOG_LEVEL_DEBUG3,
                "Committing statement: stmt_type: {}, completed: {}, children: {}, in_error: {}, "
                "success: {}",
...
StatementCache::_rollback_transaction()
{
    ...
    for (auto it = _transaction_history._history.begin(); it != _transaction_history._history.end();
         ++it) {
        QueryStmtPtr entry = it->second;
        if (entry->type == QueryStmt::PREPARE || entry->type == QueryStmt::DEALLOCATE) {
