...
static char *read_attr_value(char **input, char attr)
{
	...
	if (*begin != attr) {
		LOG_ERROR(springtail::LOG_ALL, "malformed SCRAM message (attribute \"{}\" expected", attr);
		return NULL;
	}
	begin++;

	if (*begin != '=') {
		LOG_ERROR(springtail::LOG_ALL, "malformed SCRAM message (expected \"=\" after attribute \"{}\")", attr);
		return NULL;
	}
	begin++;
...
static char *read_any_attr(char **input, char *attr_p)
{
	...
	if (!((attr >= 'A' && attr <= 'Z') ||
	      (attr >= 'a' && attr <= 'z'))) {
		LOG_ERROR(springtail::LOG_ALL, "malformed SCRAM message (attribute expected, but found invalid character \"{}\")",
				     sanitize_char(attr));
		return NULL;
	}
	if (attr_p)
	...
	if (*begin != '=') {
		LOG_ERROR(springtail::LOG_ALL, "malformed SCRAM message (expected character \"=\" after attribute \"{}\")",
				     attr);
		return NULL;
	}
	begin++;
			       ...
			       char **server_nonce_p, char **salt_p, int *saltlen_p, int *iterations_p)
{
	...
	if (strlen(server_nonce) < strlen(scram_state->client_nonce) ||
		...
		LOG_ERROR(springtail::LOG_ALL, "invalid SCRAM response (nonce mismatch)");
		goto failed;
	}
	...
	saltlen = pg_b64_decode(encoded_salt,
				...
				salt, saltlen);
	if (saltlen < 0) {
		LOG_ERROR(springtail::LOG_ALL, "malformed SCRAM message (invalid salt)");
		goto failed;
	}
	...
	if (*endptr != '\0' || iterations < 1) {
		LOG_ERROR(springtail::LOG_ALL, "malformed SCRAM message (invalid iteration count)");
		goto failed;
	}

	if (*input != '\0') {
		LOG_ERROR(springtail::LOG_ALL, "malformed SCRAM message (garbage at end of server-first-message)");
		goto failed;
	}
...
bool read_server_final_message(char *input, char *ServerSignature)
{
	...
	if (*input == 'e') {
		...
		LOG_ERROR(springtail::LOG_ALL, "error received from server in SCRAM exchange: {}", errmsg);
		goto failed;
	}
	...
	if (*input != '\0') {
		LOG_ERROR(springtail::LOG_ALL, "malformed SCRAM message (garbage at end of server-final-message)");
    }
	...
	if (server_signature_len != SCRAM_KEY_LEN) {
		LOG_ERROR(springtail::LOG_ALL, "malformed SCRAM message (malformed server signature)");
		goto failed;
	}
			       ...
			       char **client_nonce_p)
{
	...
	*cbind_flag_p = *input;
	...
	case 'p':
		...
		LOG_ERROR(springtail::LOG_ALL, "client requires SCRAM channel binding, but it is not supported");
		goto failed;
	default:
		LOG_ERROR(springtail::LOG_ALL, "malformed SCRAM message (unexpected channel-binding flag \"{}\")",
			     sanitize_char(*input));
		goto failed;
	}

	if (*input != ',') {
		LOG_ERROR(springtail::LOG_ALL, "malformed SCRAM message (comma expected, but found character \"{}\")",
			     sanitize_char(*input));
		goto failed;
	}
	input++;
	...
	if (*input == 'a') {
		LOG_ERROR(springtail::LOG_ALL, "client uses authorization identity, but it is not supported");
		goto failed;
	}
	if (*input != ',') {
		LOG_ERROR(springtail::LOG_ALL, "malformed SCRAM message (comma expected, but found character \"{}\")",
	...
	if (*input == 'm') {
		LOG_ERROR(springtail::LOG_ALL, "client requires an unsupported SCRAM extension");
		goto failed;
	}
	...
	if (!is_scram_printable(client_nonce)) {
		LOG_ERROR(springtail::LOG_ALL, "non-printable characters in SCRAM nonce");
		goto failed;
	}
			       ...
			       char **proof_p)
{
	...
	if (!(strcmp(channel_binding, "biws") == 0 && scram_state->cbind_flag == 'n') &&
		...
		LOG_ERROR(springtail::LOG_ALL, "unexpected SCRAM channel-binding attribute in client-final-message");
		goto failed;
	}
	...
	if (!value) {
		LOG_ERROR(springtail::LOG_ALL, "could not read proof");
		goto failed;
	}
	...
	if (proof == NULL) {
		LOG_ERROR(springtail::LOG_ALL, "could not decode proof");
		goto failed;
	}
	...
	if (prooflen != SCRAM_KEY_LEN) {
		LOG_ERROR(springtail::LOG_ALL, "malformed SCRAM message (malformed proof in client-final-message)");
		goto failed;
	}

	if (*input != '\0') {
		LOG_ERROR(springtail::LOG_ALL, "malformed SCRAM message (garbage at the end of client-final-message)");
		goto failed;
	}
...
char *build_server_first_message(ScramState *scram_state, const char *username, const char *stored_secret, PasswordType type)
{
	uint8_t raw_nonce[SCRAM_RAW_NONCE_LEN + 1];
	int encoded_len;
	size_t len;
				 ...
				 const char *secret)
{
	...
	if (!parse_scram_secret(secret, &iterations, &encoded_salt,
		...
		LOG_WARN(springtail::LOG_ALL, "invalid SCRAM secret for user \"{}\"", username);
		goto failed;
	}
	...
	if (saltlen < 0) {
		LOG_WARN(springtail::LOG_ALL, "malformed SCRAM secret for user \"{}\"", username);
		goto failed;
	}
