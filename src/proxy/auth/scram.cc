 ...
 */
void
free_scram_state(ScramState *scram_state)
{
...
bool
parse_scram_secret(
    const char *secret, int *iterations, char **salt, uint8_t *stored_key, uint8_t *server_key)
{
    char *s;
    char *p;
    char *scheme_str;
    ...
    *salt = strdup(salt_str);
    ...
    decoded_len =
        pg_b64_decode(storedkey_str, strlen(storedkey_str), decoded_stored_buf, decoded_len);
    ...
    decoded_len =
        pg_b64_decode(serverkey_str, strlen(serverkey_str), decoded_server_buf, decoded_len);
    if (decoded_len != SCRAM_KEY_LEN) {
        goto invalid_secret;
    }
...
get_password_type(const char *shadow_pass)
{
    ...
    if (parse_scram_secret(shadow_pass, &iterations, &encoded_salt, stored_key, server_key)) {
        free(encoded_salt);
...
char *
build_client_first_message(ScramState *scram_state)
{
...
char *
build_client_final_message(ScramState *scram_state,
                           const PgUser *user,
                           ...
                           int iterations)
{
    ...
    if (!calculate_client_proof(scram_state, user, salt, saltlen, iterations, buf, client_proof)) {
        goto failed;
    }

    len = strlcat(buf, ",p=", sizeof(buf));
    enclen = pg_b64_enc_len(sizeof(client_proof));
    enclen = pg_b64_encode((char *)client_proof, SCRAM_KEY_LEN, buf + len, enclen);
    if (enclen < 0) {
        goto failed;
    }
    len += enclen;
...
bool
read_server_first_message(ScramState *scram_state,
                          char *input,
                          char **server_nonce_p,
                          char **salt_p,
                          int *saltlen_p,
...
bool
read_server_final_message(char *input, char *ServerSignature)
{
    ...
    server_signature_len = pg_b64_decode(encoded_server_signature, strlen(encoded_server_signature),
                                         decoded_server_signature, server_signature_len);
...
static bool
calculate_client_proof(ScramState *scram_state,
                       const PgUser *user,
                       const char *salt,
                       int saltlen,
                       int iterations,
                       ...
                       uint8_t *result)
{
    ...
    if (user->has_scram_keys) {
        ...
        scramSaltedPassword(prep_password, salt, saltlen, iterations, scram_state->SaltedPassword);
    ...
    scram_HMAC_update(&ctx, scram_state->client_first_message_bare,
    ...
    scram_HMAC_update(&ctx, ",", 1);
    scram_HMAC_update(&ctx, client_final_message_without_proof,
...
bool
verify_server_signature(ScramState *scram_state, const PgUser *user, const char *ServerSignature)
{
    ...
    scram_HMAC_update(&ctx, scram_state->server_first_message,
                      strlen(scram_state->server_first_message));
    scram_HMAC_update(&ctx, ",", 1);
    scram_HMAC_update(&ctx, scram_state->client_final_message_without_proof,
...
bool
read_client_first_message(char *input,
                          char *cbind_flag_p,
...
bool
read_client_final_message(ScramState *scram_state,
                          ...
                          char **proof_p)
{
    ...
    prooflen = pg_b64_decode(encoded_proof, strlen(encoded_proof), proof, prooflen);
 ...
 */
static bool
build_adhoc_scram_secret(const char *plain_password, ScramState *scram_state)
{
    const char *password;
    char *prep_password;
    pg_saslprep_rc rc;
    ...
    scramSaltedPassword(password, saltbuf, sizeof(saltbuf), scram_state->iterations,
                        salted_password);
 ...
 */
static void
scram_mock_salt(const char *username, uint8_t *saltbuf)
{
...
static bool
build_mock_scram_secret(const char *username, ScramState *scram_state)
{
...
char *
build_server_first_message(ScramState *scram_state, const char *username, const char *stored_secret)
{
    uint8_t raw_nonce[SCRAM_RAW_NONCE_LEN + 1];
    int encoded_len;
    size_t len;
    ...
    if (!stored_secret) {
        ...
        switch (get_password_type(stored_secret)) {  // this is completely redundant...
            case PASSWORD_TYPE_SCRAM_SHA_256:
                if (!parse_scram_secret(stored_secret, &scram_state->iterations, &scram_state->salt,
                                        scram_state->StoredKey, scram_state->ServerKey)) {
                    goto failed;
                }
                break;
    ...
    len = (2 + strlen(scram_state->client_nonce) + strlen(scram_state->server_nonce) + 3 +
           strlen(scram_state->salt) + 3 + 10 + 1);
    result = (char *)malloc(len);
    if (!result) {
        goto failed;
    }
    snprintf(result, len, "r=%s%s,s=%s,i=%u", scram_state->client_nonce, scram_state->server_nonce,
...
compute_server_signature(ScramState *state)
{
    ...
    scram_HMAC_update(&ctx, state->client_first_message_bare,
                      strlen(state->client_first_message_bare));
    scram_HMAC_update(&ctx, ",", 1);
    scram_HMAC_update(&ctx, state->server_first_message, strlen(state->server_first_message));
    scram_HMAC_update(&ctx, ",", 1);
    scram_HMAC_update(&ctx, state->client_final_message_without_proof,
...
char *
build_server_final_message(ScramState *scram_state)
{
...
bool
verify_final_nonce(const ScramState *scram_state, const char *client_final_nonce)
{
    ...
    if (memcmp(client_final_nonce + client_nonce_len, scram_state->server_nonce,
               server_nonce_len) != 0) {
        return false;
    }
...
bool
verify_client_proof(ScramState *state, const char *ClientProof)
{
    uint8_t ClientSignature[SCRAM_KEY_LEN];
 ...
 */
bool
scram_verify_plain_password(const char *username, const char *password, const char *secret)
{
    char *encoded_salt = NULL;
    char *salt = NULL;
    int saltlen;
    ...
    if (!parse_scram_secret(secret, &iterations, &encoded_salt, stored_key, server_key)) {
