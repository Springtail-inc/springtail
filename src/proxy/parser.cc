...
{
    SPDLOG_INFO(
        "Context: location={}, type={}, name={}, is_read_safe={}\n  has_select_query={}, "
        "has_unsupported_query={}, has_locking={}, has_into={}, has_declare_hold={}, has_error={}, "
        "has_is_local={}",
        context.stmt_location, _stmt_names[(int8_t)context.type], context.name,
        context.is_read_safe, context.has_select_query, context.has_unsupported_query,
        context.has_locking, context.has_into, context.has_declare_hold, context.has_error,
        context.has_is_local);
...
Parser::parse_query(const std::string_view query, VerifyTableFn verify_table_fn)
{
    ...
    if (context.stmts.size() == 1) {
        ...
        PROXY_DEBUG(LOG_LEVEL_DEBUG2, "Single query is_read_safe: {}",
                    context.stmts[0]->is_read_safe);
...
Parser::_is_query_readonly(const StmtContext &context, VerifyTableFn verify_table_fn)
{
    ...
    if (_replica_unsafe_stmts.contains(context.type) || context.has_error || context.has_into ||
        context.has_locking || context.has_unsupported_query) {
        return false;
    }
...
Parser::_parse_query(const std::string_view query, ParseContext &context)
{
    ...
    PgQueryInternalParsetreeAndError tree =
        pg_query_raw_parse(query.data(), PG_QUERY_PARSE_DEFAULT);
    if (tree.error) {
        SPDLOG_ERROR("Query parse error {} at {}, for query {}", tree.error->message,
                     tree.error->cursorpos, query);
 ...
 *
 *  NOTE: when parsing a node and it has children then use _node_walker() if the child is a
 * stmt/query use raw_expression_tree_walker_impl() if the child is a list; return false to stop
 * walking the tree
 */
bool
Parser::_node_walker(Node *node, void *ctx)
{
    ...
    switch (nodeTag(node)) {
        case T_RawStmt: {  // raw stmt, first node in the parse tree; contains the actual statement
            ...
            PROXY_DEBUG(LOG_LEVEL_DEBUG2, "Parser node: rawstmt: location={}, len={}",
        ...
        case T_VariableSetStmt: {  // set variable statement; may be local set or not
            ...
            PROXY_DEBUG(LOG_LEVEL_DEBUG2, "Parser node: name {}, is_local={}, kind={}", stmt->name,
        ...
        case T_PrepareStmt: {  // prepare statement
            ...
            PROXY_DEBUG(LOG_LEVEL_DEBUG2, "Parser node: preparestmt, name={} query node tag={}",
                        stmt->name, (int)nodeTag(stmt->query));
        ...
        case T_ExecuteStmt:  // execute statement
            PROXY_DEBUG(LOG_LEVEL_DEBUG2, "Parser node: executestmt: name={}",
                        ((ExecuteStmt *)node)->name);
        ...
        case T_DeallocateStmt:  // deallocate statement
            PROXY_DEBUG(LOG_LEVEL_DEBUG2, "Parser node: deallocatestmt: name={}",
                        ((DeallocateStmt *)node)->name);
        ...
        case T_TransactionStmt: {  // transaction statement
            ...
            switch (stmt->kind) {
                ...
                case TRANS_STMT_START:
                    context->type = StmtContext::TRANSACTION_BEGIN_STMT;
                    return raw_expression_tree_walker_impl((Node *)stmt->options, _node_walker,
                                                           ctx);
        ...
        case T_FuncCall:
            ...
            raw_expression_tree_walker_impl((Node *)(((FuncCall *)node)->funcname), _node_walker,
                                            ctx);
        ...
        case T_String:
            if (parse_context->in_funccall) {
                ...
                PROXY_DEBUG(LOG_LEVEL_DEBUG2, "Parser node: funccall string {}",
                            ((String *)node)->sval);
                ...
                parse_context->func_name = parse_context->func_name.empty()
                                               ? funcname
        ...
        case T_DefElem: {
            ...
            PROXY_DEBUG(LOG_LEVEL_DEBUG2, "Parser node: name={}, action={}, argtype={}",
                        elem->defname, (int)elem->defaction, (int)nodeTag(elem->arg));
            ...
            if (strcmp("transaction_isolation", elem->defname) == 0) {
                ...
                if (nodeTag(&aconst->val) == T_String) {
                    PROXY_DEBUG(LOG_LEVEL_DEBUG2, "Parser node: defelem arg string: {}",
                                aconst->val.sval.sval);
        ...
        case T_A_Const: {
            ...
            if (nodeTag(&aconst->val) == T_String) {
                PROXY_DEBUG(LOG_LEVEL_DEBUG2, "Parser node: aconst string: {}",
                            aconst->val.sval.sval);
                _set_string(aconst->val.sval.sval, context->name);
            }
...
Parser::_check_stmt(Node *node)
{
    ...
    // see:
    // https://github.com/postgres/postgres/blob/d12b4ba1bd3eedd862064cf1dad5ff107c5cba90/src/backend/tcop/utility.c#L127
    switch (nodeTag(node)) {
        ...
        default:
            PROXY_DEBUG(LOG_LEVEL_DEBUG2, "Parser: rejecting statement type {}",
                        (int)nodeTag(node));
            return false;
    }
}
