...
ssl_info_callback(const SSL *s, int where, int ret)
{
    ...
    if (where & SSL_CB_HANDSHAKE_START) {
        ...
        PROXY_DEBUG(LOG_LEVEL_DEBUG4, "SSL info (CB_LOOP): fd={}, {}:{}", fd, str,
        ...
        if (ret == 0) {
            PROXY_DEBUG(LOG_LEVEL_DEBUG4, "SSL info (CB_EXIT): fd={}, {}:failed in {}", fd, str,
                        SSL_state_string_long(s));
        } else if (ret < 0) {
            PROXY_DEBUG(LOG_LEVEL_DEBUG4, "SSL info (CB_EXIT): fd={}, {}:error in {}", fd, str,
                        SSL_state_string_long(s));
        }
    } else {
        PROXY_DEBUG(LOG_LEVEL_DEBUG4, "SSL info callback: fd={}, where={:#X}, ret={}", fd, where,
                    ret);
    }
}
...
ProxyServer::_log_connect(SessionPtr session)
{
    ...
    PROXY_DEBUG(LOG_LEVEL_DEBUG3, "{} connected from: {} on socket {}",
                (session->type() == Session::Type::CLIENT ? "Client" : "Server"), endpoint,
                session->get_connection()->get_socket());
...
ProxyServer::_do_accept()
{
    while (true) {
        ...
        int client_socket =
            accept(_socket, (struct sockaddr *)&client_address, &client_address_size);
        if (client_socket == -1) {
        ...
        ProxyConnectionPtr connection =
            std::make_shared<ProxyConnection>(client_socket, client_address);
...
ProxyServer::run()
{
    while (!_shutdown) {
        ...
        for (auto &session : runnable_sessions) {
            PROXY_DEBUG(LOG_LEVEL_DEBUG4, "Queueing session: [{}:{}]", session->type_str(),
                        session->id());
...
void
ProxyServer::register_session(SessionPtr session, bool waiting_session_insert)
{
