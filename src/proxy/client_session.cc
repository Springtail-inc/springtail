...
ClientSession::ClientSession(ProxyConnectionPtr connection, ProxyServerPtr server)
...
{
    PROXY_DEBUG(LOG_LEVEL_DEBUG1, "[C:{}] Client connected: endpoint={}", _id,
                connection->endpoint());
...
ClientSession::_process_msg(SessionMsgPtr msg)
{
    ...
    PROXY_DEBUG(LOG_LEVEL_DEBUG1, "[C:{}] Client session got message from server session: {}", _id,
                msg->type_str());

    // entry point for messages from server session
    switch (msg->type()) {
        case SessionMsg::MSG_SERVER_CLIENT_AUTH_DONE:
            PROXY_DEBUG(LOG_LEVEL_DEBUG1, "[C:{}] Client session got auth done from server session",
                        _id);
            if (_state == READY) {
            ...
            if (_shadow_mode && _replica_session == nullptr) {
                ...
                PROXY_DEBUG(LOG_LEVEL_DEBUG1,
                            "[C:{}] Creating replica session in shadow mode: seq_id={}", _id,
                            seq_id);
        ...
        case SessionMsg::MSG_SERVER_CLIENT_FATAL_ERROR:
            PROXY_DEBUG(LOG_LEVEL_DEBUG1,
                        "[C:{}] Client session got fatal error from server session", _id);
        ...
        case SessionMsg::MSG_SERVER_CLIENT_READY: {
            PROXY_DEBUG(LOG_LEVEL_DEBUG1,
                        "[C:{}] Client session got ready from server session: status={}", _id,
    ...
    PROXY_DEBUG(LOG_LEVEL_DEBUG3,
                "[C:{}] Client session done with msg, in_transaction={}, empty={}", _id,
                _in_transaction, is_msg_queue_empty());
    ...
    if (!_in_transaction && is_msg_queue_empty()) {
        ...
        PROXY_DEBUG(LOG_LEVEL_DEBUG2, "[C:{}] Clearing server session: id={}", _id,
                    server_session->id());
    }
}
...
ClientSession::_process_connection()
{
    ...
    PROXY_DEBUG(LOG_LEVEL_DEBUG1, "[C:{}] Processing packet, client session: state={:d}", _id,
                (int8_t)_state);
...
ClientSession::_handle_scram_auth(const std::string_view data, uint64_t seq_id)
{
    ...
    raw[data.size()] = '\0';
    if (!read_client_first_message(raw, &_login->scram_state.cbind_flag,
...
ClientSession::_handle_scram_auth_continue(const std::string_view data, uint64_t seq_id)
{
    ...
    if (!read_client_final_message(&_login->scram_state,
                                   reinterpret_cast<const uint8_t *>(data.data()), raw,
...
void
ClientSession::_encode_parameter_status(BufferPtr buffer,
...
ClientSession::_handle_request()
{
    ...
    PROXY_DEBUG(LOG_LEVEL_DEBUG1, "[C:{}] Client handle request: buffers={}", _id,
                blist.buffers.size());
    int i = 0;
    for (auto buffer : blist.buffers) {
        char code = buffer->get();
        ...
        PROXY_DEBUG(LOG_LEVEL_DEBUG1, "[C:{}] Client, buf={}, got request code: {}, seq_id: {}",
                    _id, i++, code, seq_id);
        ...
        switch (code) {
            ...
            case 'f':  // copy fail
                ...
                PROXY_DEBUG(LOG_LEVEL_DEBUG1, "[C:{}] Forwarding to server: code={}, len={}", _id,
                            code, len);
...
ClientSession::_handle_parse(BufferPtr buffer, uint64_t seq_id)
{
    ...
    // parse the query
    std::vector<Parser::StmtContextPtr> &&parse_contexts =
        Parser::parse_query(query, [this](const std::string &schema, const std::string &table) {
            return DatabaseMgr::get_instance()->is_table_replicated(
                this->_db_id, this->_default_schema, schema, table);
        });

    // Create a query statement object
    QueryStmt::Type qs_type =
        parse_contexts.empty() ? QueryStmt::Type::PREPARE : _remap_parse_type(parse_contexts[0]);
    bool is_read_safe = parse_contexts.empty() ? true : parse_contexts[0]->is_read_safe;
    QueryStmtPtr query_stmt =
        std::make_shared<QueryStmt>(QueryStmt::Type::PREPARE, buffer, is_read_safe, stmt.data());
...
ClientSession::_handle_bind(BufferPtr buffer, uint64_t seq_id)
{
    ...
    QueryStmtPtr qs =
        _stmt_cache.add(QueryStmt::DECLARE, buffer, prepared_stmt->is_read_safe, portal.data());
...
ClientSession::_handle_describe(BufferPtr buffer, uint64_t seq_id)
{
    ...
    PROXY_DEBUG(LOG_LEVEL_DEBUG2, "[C:{}] Describe request: type={}, name={}", _id, stmt_type,
                name);
...
ClientSession::_handle_execute(BufferPtr buffer, uint64_t seq_id)
{
    ...
    if (query_stmt != nullptr) {
        ...
        if (dep_stmt != query_stmt && dep_stmt->type == QueryStmt::PREPARE &&
...
ClientSession::_handle_sync(BufferPtr buffer, uint64_t seq_id)
{
    ...
    QueryStmtPtr qs =
        std::make_shared<QueryStmt>(QueryStmt::SYNC, buffer, associated_session_type() == REPLICA);
...
{
    PROXY_DEBUG(LOG_LEVEL_DEBUG1, "[C:{}] Selecting server session: type={}", _id,
                type == PRIMARY ? "PRIMARY" : "REPLICA");
    ...
    if (get_associated_session() != nullptr) {
        ...
        ServerSessionPtr session =
            std::static_pointer_cast<ServerSession>(get_associated_session());
    ...
    if (type == PRIMARY && _primary_session != nullptr) {
        // use primary session
        PROXY_DEBUG(LOG_LEVEL_DEBUG3, "[C:{}] Using primary session; setting associated session",
                    _id);
    ...
    if (type == REPLICA && _replica_session != nullptr) {
        // use replica session
        PROXY_DEBUG(LOG_LEVEL_DEBUG3, "[C:{}] Using replica session; setting associated session",
                    _id);
        session = _replica_session;
        assert(!_shadow_mode);
        return session;
    ...
    PROXY_DEBUG(LOG_LEVEL_DEBUG1, "[C:{}] Creating new server session: type={}", _id,
                type == PRIMARY ? "PRIMARY" : "REPLICA");
...
ClientSession::_create_server_session(Session::Type type, uint64_t seq_id)
{
    ...
    if (session == nullptr) {
        ...
        PROXY_DEBUG(LOG_LEVEL_DEBUG2, "[C:{}] Allocating new server session: {}:{}", _id, _database,
                    _user->username());

        from_pool = false;

        if ((session = db_mgr->allocate_session(_server, type, _db_id, _user, _parameters)) ==
            nullptr) {
            SPDLOG_ERROR("Failed to allocate server session for user {}, database {}",
                         _user->username(), _database);
            assert(0);
            return nullptr;
        }
    }

    PROXY_DEBUG(LOG_LEVEL_DEBUG1, "[C:{}] Got server session: id={}, is_ready={}", _id,
                session->id(), session->is_ready());
    ...
    if (from_pool) {
        ...
        PROXY_DEBUG(LOG_LEVEL_DEBUG3, "[C:{}] Applying session parameters to server session: id={}",
                    _id, session->id());
        BufferPtr buffer = _encode_session_param_query();
        SessionMsgPtr param_msg =
            SessionMsg::create(SessionMsg::MSG_CLIENT_SERVER_INIT_PARAMS, buffer, seq_id);
                                   ...
                                   std::vector<QueryStmtPtr> &dependencies)
{
    ...
    for (auto &context : parse_contexts) {
        ...
        QueryStmtPtr stmt = std::make_shared<QueryStmt>(
            stmt_type, p_query.data(), context->is_read_safe, context->name.data());

        // add to parent
        qs->children.push_back(stmt);

        // if there is a dependency add it; only prepared stmts; only add if not in current
        // transaction
