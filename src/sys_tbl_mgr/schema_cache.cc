...
SchemaMetadataPtr
SchemaCache::get(uint64_t db, uint64_t tid, const XidLsn &xid, PopulateFn populate)
{
    std::unique_lock lock(_mutex);
    auto key = std::make_pair(db, tid);

    LOG_DEBUG(LOG_SCHEMA, LOG_LEVEL_DEBUG1, "db {}, table {}, xid {}:{}", db, tid, xid.xid, xid.lsn);
    ...
    while (true) {
        ...
        if (schema_i != _schema_map.end()) {
            ...
            if (entry->fetching) {
                ...
                entry->cond.wait(lock, [&entry]() { return !entry->fetching; });
        ...
        if (!entry->invalidated) {
            _populate_index_map_locked(db, tid, entry->schema->indexes);
        }
...
void
SchemaCache::_populate_index_map_locked(uint64_t db,
                                        uint64_t tid,
                           ...
                           PopulateFn populate)
{
    ...
    SchemaMetadataPtr metadata;
    {
        ...
        LOG_DEBUG(LOG_SCHEMA, LOG_LEVEL_DEBUG1, "db {}, table {}, start_xid {}:{}, end_xid {}:{}", db, tid,
...
void
SchemaCache::invalidate_table(uint64_t db, uint64_t tid, const XidLsn &xid)
{
    std::unique_lock lock(_mutex);
...
void
SchemaCache::invalidate_db(uint64_t db, const XidLsn &xid)
{
    std::unique_lock lock(_mutex);
...
void
SchemaCache::invalidate_by_index(uint64_t db, uint64_t index_id, const XidLsn &xid)
{
    std::unique_lock lock(_mutex);
