...
void
Service::create_index(DDLStatement &_return, const IndexRequest &request)
{
    SPDLOG_INFO("got create_index(): db {}, table {}, index {}, xid {}:{}", request.db_id,
...
Service::_get_index_info(const GetIndexInfoRequest &request)
{
    ...
    if (!info) {
        SPDLOG_DEBUG_MODULE(LOG_SCHEMA, "Index not found: {}@{} - {}", request.db_id, request.xid,
                            request.index_id);
        IndexInfo dummy;
        dummy.id = 0;
        return dummy;
...
Service::_set_index_state(const SetIndexStateRequest &request)
{
    ...
    auto index_i = std::ranges::find_if(info->indexes,
                                        [&](auto const &v) { return request.index_id == v.id; });
    ...
    auto tuple = sys_tbl::IndexNames::Data::tuple(
        index_info.schema, index_info.name, index_info.table_id, request.index_id, xid.xid, xid.lsn,
        static_cast<sys_tbl::IndexNames::State>(request.state), index_info.is_unique);
...
void
Service::set_index_state(Status &_return, const SetIndexStateRequest &request)
{
...
Service::_create_index(const IndexRequest &request)
{
    ...
    // update index names
    {
        ...
        auto index_names_t = _get_mutable_system_table(request.db_id, sys_tbl::IndexNames::ID);
        auto tuple = sys_tbl::IndexNames::Data::tuple(
            request.index.schema, request.index.name, request.index.table_id, request.index.id,
...
void
Service::drop_index(DDLStatement &_return, const DropIndexRequest &request)
{
    SPDLOG_INFO("got drop_index(): db {}, index {}, xid {}:{}", request.db_id, request.index_id,
                request.xid, request.lsn);
                     ...
                     std::optional<uint64_t> tid)
{
    ...
    if (static_cast<sys_tbl::IndexNames::State>(info->first.state) ==
    ...
    SPDLOG_DEBUG_MODULE(LOG_SCHEMA, "Drop index found {}:{} -- {}", db_id, info->first.table_id,
                        index_id);
    auto index_names_t = _get_mutable_system_table(db_id, sys_tbl::IndexNames::ID);
    auto tuple = sys_tbl::IndexNames::Data::tuple(
        info->first.schema, info->first.name, info->first.table_id, index_id, xid.xid, xid.lsn,
        sys_tbl::IndexNames::State::DELETED, info->first.is_unique);
    index_names_t->upsert(tuple, xid.xid, constant::UNKNOWN_EXTENT);
}

void
Service::create_table(DDLStatement &_return, const TableRequest &request)
{
    SPDLOG_INFO("got create_table() -- db {} table {} xid {} lsn {}", request.db_id,
...
Service::_create_table(const TableRequest &request)
{
    ...
    _set_schema_info(request.db_id, request.table.id, request.table.name, request.table.schema,
                     columns);
...
void
Service::alter_table(DDLStatement &_return, const TableRequest &request)
{
    ...
    if (table_info->schema != request.table.schema || table_info->name != request.table.name) {
        ...
        auto new_info = std::make_shared<TableInfo>(request.table.id, request.xid, request.lsn,
                                                    request.table.schema, request.table.name, true);
        ...
        if (history.update_type != static_cast<int8_t>(SchemaUpdateType::NO_CHANGE) &&
            ...
            _set_schema_info(request.db_id, request.table.id, request.table.name,
                             request.table.schema, {history});
        }
    }
...
void
Service::drop_table(DDLStatement &_return, const DropTableRequest &request)
{
...
Service::_drop_table(const DropTableRequest &request)
{
    ...
    auto table_info = std::make_shared<TableInfo>(request.table_id, request.xid, request.lsn,
                                                  request.schema, request.name, false);
    _set_table_info(request.db_id, table_info);
...
void
Service::update_roots(Status &_return, const UpdateRootsRequest &request)
{
...
void
Service::finalize(Status &_return, const FinalizeRequest &request)
{
    ...
    if (md_map.empty()) {
        SPDLOG_INFO("Nothing to finalize: {}@{} >= {}", request.db_id, request.xid, write_xid);
        return;
    }
...
void
Service::get_roots(GetRootsResponse &_return, const GetRootsRequest &request)
{
...
void
Service::get_schema(GetSchemaResponse &_return, const GetSchemaRequest &request)
{
...
void
Service::get_target_schema(GetSchemaResponse &_return, const GetTargetSchemaRequest &request)
{
                         ...
                         const UpdateRootsRequest &roots)
{
    ...
    SPDLOG_DEBUG_MODULE(LOG_SCHEMA, "Create table: {}:{} @ {}:{}", create.db_id, create.table.id,
                        create.xid, create.lsn);
    ...
    SPDLOG_DEBUG_MODULE(LOG_SCHEMA, "Update roots: {}:{} @ {}:{}", create.db_id, create.table.id,
                        create.xid, create.lsn);
...
void
Service::_set_table_info(uint64_t db_id, TableInfoPtr table_info)
{
    ...
    auto tuple =
        sys_tbl::TableNames::Data::tuple(table_info->schema, table_info->name, table_info->id,
...
Service::_get_roots_info(uint64_t db_id, uint64_t table_id, const XidLsn &xid)
{
    ...
    for (; rrow_i != roots_t->end(); ++rrow_i) {
        ...
        auto it = std::ranges::find_if(roots_info->roots,
                                       [&ri](auto const &v) { return v.index_id == ri.index_id; });
                              ...
                              const XidLsn &access_xid)
{
    ...
    for (auto names_i = names_t->lower_bound(search_key); names_i != names_t->end(); ++names_i) {
        ...
        if (access_xid < index_xid) {
            ...
            if (XidLsn(schema_info->access_xid_end, schema_info->access_lsn_end) > index_xid) {
        ...
        if (XidLsn(schema_info->access_xid_start, schema_info->access_lsn_start) < index_xid) {
                          ...
                          const std::vector<ColumnHistory> &columns)
{
    std::map<uint32_t, uint32_t>
        primary_keys;  // record the primary keys to update the indexes table
    ...
    if (!primary_keys.empty()) {
        ...
        auto index_names_t = _get_mutable_system_table(db_id, sys_tbl::IndexNames::ID);
        auto tuple = sys_tbl::IndexNames::Data::tuple(schema, table_name + ".primary_key", table_id,
                                                      constant::INDEX_PRIMARY,  // index id
                                                      xid.xid, xid.lsn,
                                                      sys_tbl::IndexNames::State::READY, true);
        index_names_t->upsert(tuple, write_xid, constant::UNKNOWN_EXTENT);
