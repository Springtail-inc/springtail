...
std::filesystem::path
_get_table_dir(const std::filesystem::path &base,
               uint64_t db_id,
               uint64_t table_id,
      ...
      _stats(metadata.stats)
{
    ...
    if (roots.empty()) {
        if (std::filesystem::exists(_table_dir / constant::ROOTS_FILE)) {
            ...
            for (auto &row : *extent) {
                roots.push_back(
                    {_roots_index_id_f->get_uint64(row), _roots_root_f->get_uint64(row)});
            }
    ...
    auto it = std::ranges::find_if(
        roots, [](auto const &v) { return v.index_id == constant::INDEX_PRIMARY; });
    assert(it != roots.end());

    _primary_index = std::make_shared<BTree>(_table_dir / constant::INDEX_PRIMARY_FILE, xid,
                                             primary_schema, it->extent_id);
    ...
    for (auto const &idx : secondary) {
        ...
        if (!idx_cols.empty()) {
            auto it =
                std::ranges::find_if(roots, [&](auto const &v) { return v.index_id == idx.id; });
            assert(it != roots.end());
...
BTreePtr
Table::_create_index_root(uint64_t index_id,
      ...
      _for_gc(for_gc)
{
    ...
    if (primary_key.empty()) {
        ...
        _primary_index =
            std::make_shared<MutableBTree>(_table_dir / constant::INDEX_PRIMARY_FILE,
                                           non_primary_key, primary_schema, _target_xid);
    } else {
        primary_schema = _schema->create_schema(primary_key, {extent_c}, primary_key);

        _primary_index = std::make_shared<MutableBTree>(_table_dir / constant::INDEX_PRIMARY_FILE,
                                                        primary_key, primary_schema, _target_xid);
    }
    ...
    _primary_lookup = std::make_shared<BTree>(_table_dir / constant::INDEX_PRIMARY_FILE, access_xid,
                                              primary_schema, it->extent_id);
...
void
MutableTable::insert(TuplePtr value, uint64_t xid, uint64_t extent_id)
{
...
void
MutableTable::upsert(TuplePtr value, uint64_t xid, uint64_t extent_id)
{
...
void
MutableTable::remove(TuplePtr key, uint64_t xid, uint64_t extent_id)
{
    // perform the removal
    if (extent_id == constant::UNKNOWN_EXTENT) {
        if (_primary_key.empty()) {
...
void
MutableTable::update(TuplePtr value, uint64_t xid, uint64_t extent_id)
{
    if (extent_id == constant::UNKNOWN_EXTENT) {
        // note: cannot perform an update() with no primary key, should be split into a remove() and
        // insert()
...
{
    return StorageCache::get_instance()->get(
        _data_file, extent_id, _access_xid, _target_xid, false,
...
MutableTable::_flush_and_populate_indexes(StorageCache::PagePtr::element_type *page)
{
    ...
    for (auto extent_id : offsets) {
        ...
        // go through each row and pass the relevant key to each of the secondary indexes for
        // insertion
        ...
        for (auto &row : *new_page) {
            ...
            for (auto const &[index_id, idx] : _secondary_indexes) {
                ...
                // note: uncomment if you need to debug the entries being populated into the
                // secondary indexes SPDLOG_DEBUG_MODULE(LOG_BTREE, "Secondary populate {}",
                // svalue->to_string());
...
MutableTable::finalize()
{
    ...
    auto root_handle =
        IOMgr::get_instance()->open(_table_dir / filename, IOMgr::IO_MODE::APPEND, true);
    ...
    // swap the symlink
    std::filesystem::create_symlink(_table_dir / filename, _table_dir / constant::ROOTS_TMP_FILE);
...
MutableTable::create_index_root(uint64_t index_id, const std::vector<uint32_t> &index_columns)
{
    ...
    auto btree = std::make_shared<MutableBTree>(
        _table_dir / fmt::format(constant::INDEX_FILE, index_id), key, index_schema, _target_xid);
    return btree;
}

void
MutableTable::_insert_direct(TuplePtr value, uint64_t xid, uint64_t extent_id)
{
    // get the page from the cache
    auto page = StorageCache::get_instance()->get(
        _data_file, extent_id, _access_xid, _target_xid, false,
        [this](StorageCache::PagePtr page) { return _flush_handler(page); });
...
void
MutableTable::_insert_empty(TuplePtr value, uint64_t xid)
{
    // get the page from the cache if we don't have one
    if (!_empty_page) {
        _empty_page = std::make_unique<StorageCache::SafePagePtr>(StorageCache::get_instance()->get(
            _data_file, constant::UNKNOWN_EXTENT, _access_xid, _target_xid));
    }
...
void
MutableTable::_append_empty(TuplePtr value, uint64_t xid)
{
    // get the page from the cache if we don't have one
    if (!_empty_page) {
        _empty_page = std::make_unique<StorageCache::SafePagePtr>(StorageCache::get_instance()->get(
...
void
MutableTable::_insert_append(TuplePtr value, uint64_t xid)
{
...
void
MutableTable::_insert_by_lookup(TuplePtr value, uint64_t xid)
{
    assert(!_primary_key.empty());
...
bool
MutableTable::_upsert_direct(TuplePtr value, uint64_t xid, uint64_t extent_id)
{
    // get the page from the cache
    auto page = StorageCache::get_instance()->get(
        _data_file, extent_id, _access_xid, _target_xid, false,
...
bool
MutableTable::_upsert_empty(TuplePtr value, uint64_t xid)
{
    // get the page from the cache if we don't have one
    if (!_empty_page) {
        _empty_page = std::make_unique<StorageCache::SafePagePtr>(StorageCache::get_instance()->get(
...
bool
MutableTable::_upsert_by_lookup(TuplePtr value, uint64_t xid)
{
    assert(!_primary_key.empty());
...
void
MutableTable::_remove_direct(TuplePtr value, uint64_t xid, uint64_t extent_id)
{
    // get the page from the cache
    auto page = StorageCache::get_instance()->get(
        _data_file, extent_id, _access_xid, _target_xid, false,
...
void
MutableTable::_remove_empty(TuplePtr value, uint64_t xid)
{
    // get the page from the cache if we don't have one
    if (!_empty_page) {
        _empty_page = std::make_unique<StorageCache::SafePagePtr>(StorageCache::get_instance()->get(
...
void
MutableTable::_remove_by_lookup(TuplePtr key, uint64_t xid)
{
    // if the primary_lookup tree is empty, we will maintain a single page of data that we will
    // keep against the table and use for all operations.
    if (_primary_lookup->empty()) {
...
void
MutableTable::_remove_by_scan(TuplePtr value, uint64_t xid)
{
...
void
MutableTable::_update_direct(TuplePtr value, uint64_t xid, uint64_t extent_id)
{
    // get the page from the cache
    auto page = StorageCache::get_instance()->get(
        _data_file, extent_id, _access_xid, _target_xid, false,
    ...
    // note: this can only be used when a primary key is present, otherwise update should have been
    // split
    page->update(value, _schema);
}

void
MutableTable::_update_empty(TuplePtr value, uint64_t xid)
{
    // get the page from the cache if we don't have one
    if (!_empty_page) {
        _empty_page = std::make_unique<StorageCache::SafePagePtr>(StorageCache::get_instance()->get(
...
void
MutableTable::_update_by_lookup(TuplePtr value, uint64_t xid)
{
    assert(!_primary_key.empty());
...
void
Table::Iterator::Secondary::update_page()
{
...
const Extent::Row &
Table::Iterator::Secondary::row() const
{
    return *_page_i;
}
...
{
    if (index_id == constant::INDEX_PRIMARY) {
        _tracker.emplace<Primary>(table, table->_primary_index, table->_primary_index->end(),
                                  StorageCache::SafePagePtr{}, StorageCache::Page::Iterator{});
    } else {
        auto const &[btree, cols] = table->_secondary_indexes.at(index_id);
        auto index_schema = _create_index_schema(table->_schema, cols);
        _tracker.emplace<Secondary>(table, btree, btree->end(), index_schema);
    }
}
}  // namespace springtail
