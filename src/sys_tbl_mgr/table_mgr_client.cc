...
namespace springtail {
    ...
    TableMgrClient::get_extent_schema(uint64_t db_id, uint64_t table_id,
                                      const XidLsn &xid, bool allow_undefined, bool include_internal_row_id)
    {
        if (table_id < constant::MAX_SYSTEM_TABLE_ID) {
        ...
        return std::make_shared<ExtentSchema>(meta->columns, allow_undefined, include_internal_row_id);
    }
