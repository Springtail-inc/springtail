...
SchemaMgr::SchemaMgr()
{
    ...
    // TableNames
    _system_cache[{sys_tbl::TableNames::ID, constant::INDEX_DATA, true}] =
        std::make_shared<ExtentSchema>(sys_tbl::TableNames::Data::SCHEMA);

    // TableRoots
    _system_cache[{sys_tbl::TableRoots::ID, constant::INDEX_DATA, true}] =
        std::make_shared<ExtentSchema>(sys_tbl::TableRoots::Data::SCHEMA);

    // Indexes
    _system_cache[{sys_tbl::Indexes::ID, constant::INDEX_DATA, true}] =
        std::make_shared<ExtentSchema>(sys_tbl::Indexes::Data::SCHEMA);

    // Schemas
    _system_cache[{sys_tbl::Schemas::ID, constant::INDEX_DATA, true}] =
        std::make_shared<ExtentSchema>(sys_tbl::Schemas::Data::SCHEMA);

    // TableStats
    _system_cache[{sys_tbl::TableStats::ID, constant::INDEX_DATA, true}] =
        std::make_shared<ExtentSchema>(sys_tbl::TableStats::Data::SCHEMA);

    // IndexNames
    _system_cache[{sys_tbl::IndexNames::ID, constant::INDEX_DATA, true}] =
        std::make_shared<ExtentSchema>(sys_tbl::IndexNames::Data::SCHEMA);
}
                      ...
                      const XidLsn &target_xid)
{
    ...
    auto &&meta = sys_tbl_mgr::Client::get_instance()->get_target_schema(db_id, table_id,
...
SchemaMgr::get_extent_schema(uint64_t db_id, uint64_t table_id, const XidLsn &xid)
{
    // first check if it's an immutable system table schema
    auto &&system_i = _system_cache.find({table_id, constant::INDEX_DATA, true});
    if (system_i != _system_cache.end()) {
