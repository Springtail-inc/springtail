...
main(int argc, char *argv[])
{
    uint64_t max_xid = 0;
    bool all_xids = false;
    std::optional<uint64_t> db_id = std::nullopt;
    ...
    desc.add_options()("db_id,d", po::value<uint64_t>()->notifier([&db_id](uint64_t v) { db_id = v; }), "Database id to check");
    ...
    auto fs_check = std::make_shared<test::FSCheck>(db_id, max_xid, all_xids);
    fs_check->check_dbs();
    fs_check.reset();
