...
void
SysTblMgr_Test::_set_index_state(uint64_t table_id,
                                 uint64_t index_id,
...
PgMsgDropIndex
SysTblMgr_Test::_drop_index(uint64_t index_id)
{
    auto xid = _next_lsn();
...
PgMsgIndex
SysTblMgr_Test::_create_index(uint64_t tid, const std::string &name, uint64_t index_id)
{
    auto xid = _next_lsn();
...
PgMsgTable
SysTblMgr_Test::_create_table(uint64_t tid, const std::string &name)
{
    auto xid = _next_lsn();
    ...
    create_msg.columns.push_back(
        {"col1", static_cast<uint8_t>(SchemaType::TEXT), 0, "foo", 1, 0, false, true});
    create_msg.columns.push_back(
        {"col2", static_cast<uint8_t>(SchemaType::INT32), 0, std::nullopt, 2, 0, true, false});
...
void
SysTblMgr_Test::_drop_table(uint64_t tid, const std::string &name)
{
    auto xid = _next_lsn();
...
TEST_F(SysTblMgr_Test, Basic) { _client->ping(); }
...
TEST_F(SysTblMgr_Test, Complex)
{
    ...
    // add a column col3
    msg.columns.push_back(
        {"colthree", static_cast<uint8_t>(SchemaType::INT32), 0, std::nullopt, 3, 0, true, false});
    _alter_table(msg);
...
TEST_F(SysTblMgr_Test, Threaded)
{
    ...
    auto on_completion = [this]() noexcept { _finalize(); };
    ...
    auto work_fn = [&](uint64_t tid, PgMsgTable &msg) {
        for (int i = 0; i < loop_count; i++) {
            msg.columns.push_back({"col3", static_cast<uint8_t>(SchemaType::INT32), 0, std::nullopt,
                                   3, 0, true, false});
            _alter_table(msg);
    ...
    auto get_fn = [&](uint64_t tid) {
        for (int i = 0; i < loop_count; i++) {
            ...
            for (int j = 0; j < 10; ++j) {
                auto &&result = _client->get_target_schema(
                    _db, tid, access_xid, {_xid.xid, std::max(_xid.lsn - 1, (uint64_t)0)});
            }
    ...
    threads.reserve(4);
    threads.emplace_back(std::thread([&]() { work_fn(tids[0], t1msg); }));
    threads.emplace_back(std::thread([&]() { work_fn(tids[1], t2msg); }));
    threads.emplace_back(std::thread([&]() { get_fn(tids[0]); }));
    threads.emplace_back(std::thread([&]() { get_fn(tids[1]); }));
