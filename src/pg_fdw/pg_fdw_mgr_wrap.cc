...
{
    if (state) {
        return get_fdw_mgr()->fdw_begin_scan(static_cast<PgFdwState *>(state), target_list,
                                             qual_list, sortgroup);
    }
}
...
bool
fdw_iterate_scan(
    void *state, int num_attrs, Form_pg_attribute *attrs, Datum *values, bool *nulls, bool *eos)
{
    if (state) {
        return get_fdw_mgr()->fdw_iterate_scan(static_cast<PgFdwState *>(state), num_attrs, attrs,
                                               values, nulls, eos);
    }
    return false;
}
...
List *
fdw_import_foreign_schema(const char *server,
                          const char *schema,
