    ...
    auto find_qual = [&qual_list](auto pos) -> ConstQualPtr {
        ...
        foreach (lc, qual_list) {
            ...
            if (PgFdwMgr::_is_type_sortable(qual->base.typeoid, qual->base.op) &&
                qual->base.isArray == false && qual->base.varattno == pos) {
                return qual;
            }
        }
        return nullptr;
...
PgFdwState *
PgFdwMgr::fdw_create_state(uint64_t db_id, uint64_t tid, uint64_t pg_xid, uint64_t schema_xid)
{
    ...
    SPDLOG_DEBUG_MODULE(LOG_FDW, "fdw_create_state: db_id: {}, tid: {}, xid: {}, pg_xid: {}", db_id,
                        tid, xid, pg_xid);
...
PgFdwMgr::fdw_begin_scan(PgFdwState *state, List *target_list, List *qual_list, List *sortgroup)
{
    ...
    foreach (lc, target_list) {
        ...
        SPDLOG_DEBUG_MODULE(LOG_FDW, "Target list column: {}:{}", attno,
                            state->columns[attno].name);
    }
...
{
    SPDLOG_DEBUG_MODULE(LOG_FDW, "fdw_end: tid: {}, rows fetched: {}, rows skipped: {}", state->tid,
                        state->rows_fetched, state->rows_skipped);
    delete state;
}
    ...
    *eos = false;
    ...
    if (*state->iter_start == *state->iter_end) {
        ...
        if (state->scan_asc) {
            ...
            if (state->index.has_value() &&
                state->iter_end != state->table->end(state->index->id)) {
            ...
            if (state->index.has_value() &&
                state->iter_start != state->table->begin(state->index->id)) {
                auto tuple = std::make_shared<FieldTuple>(state->qual_fields, nullptr);
                state->iter_start.emplace(state->table->begin(state->index->id));
                state->iter_end.emplace(state->table->lower_bound(tuple, state->index->id));
                return false;
    ...
    if (state->qual_fields != nullptr) {
        for (int i = 0; i < state->filtered_quals.size(); i++) {
            ...
            bool res = _compare_field(row, state->fields->at(field_idx), state->qual_fields->at(i),
                                      qual->base.op);
            if (res) {
                continue;
            }
...
PgFdwMgr::fdw_can_sort(SpringtailPlanState *state, List *sortgroup)
{
    ...
    // and null_first/reverse combination is supported
    {
        ...
        foreach (lc, sortgroup) {
            ...
            if (i == 0) {
                ...
                SPDLOG_DEBUG_MODULE(LOG_FDW,
                                    "The sort order must be the same for all attributes: {}!={}",
                                    reversed, pathkey->reversed);
                return {};
            }
            ...
            if (!(pathkey->nulls_first ? pathkey->reversed : !pathkey->reversed)) {
                SPDLOG_DEBUG_MODULE(LOG_FDW,
                                    "This combination isn't supported: null_first={}, reversed={}",
    ...
    auto it = std::find_if(pg_state->indexes.begin(), pg_state->indexes.end(),
                           [](auto const &idx) { return idx.id == constant::INDEX_PRIMARY; });
...
PgFdwMgr::fdw_get_path_keys(SpringtailPlanState *state)
{
    ...
    for (auto const &idx : pg_state->indexes) {
        ...
        item = lappend(item, makeConst(INT4OID, -1, InvalidOid, 4, rows, false, true));
...
void
PgFdwMgr::fdw_get_rel_size(
    SpringtailPlanState *planstate, List *target_list, List *qual_list, double *rows, int *width)
{
                                ...
                                int32_t atttypmod)
{
    switch (field->get_type()) {
        ...
        case SchemaType::BINARY: {
            ...
            Datum value_datum = OidFunctionCall3(typeinput, datum, ObjectIdGetDatum(pg_type),
                             ...
                             std::vector<std::tuple<std::string, std::string, bool>> &columns)
{
    ...
    create += fmt::format("\n) SERVER {} OPTIONS (tid '{}');",
                          quote_identifier(server_name.c_str()), tid);
                                    ...
                                    uint64_t schema_xid)
{
    ...
    auto table = TableMgr::get_instance()->get_table(db_id, sys_tbl::TableNames::ID, schema_xid);
    // get field array
    ...
    for (auto row : (*table)) {
        ...
        SPDLOG_DEBUG_MODULE(LOG_FDW, "Found table {}.{} tid={}, xid={}", schema_name, table_name,
                            tid, xid);

        // lookup table in map, if found the xid if it is newer
        auto entry = table_map.insert({table_name, {tid, xid}});
        if (entry.second == false) {
            SPDLOG_DEBUG_MODULE(LOG_FDW, "Table {} already exists in schema {}", table_name,
                                schema_name);
    ...
    for (auto row : (*table)) {
        ...
        if (tid != current_tid) {
            if (!current_table.empty()) {
                // dump this table
                std::string sql =
                    _gen_fdw_table_sql(server, schema, current_table, current_tid, columns);
                commands = lappend(commands, pstrdup(sql.c_str()));
            }
...
bool
PgFdwMgr::_compare_field(const std::any &row, FieldPtr val_field, FieldPtr key_field, QualOpName op)
{
    ...
    : table(table), tid(tid), xid(xid), stats(table->get_stats())
{
    ...
    if (tid > constant::MAX_SYSTEM_TABLE_ID) {
        auto &&meta = sys_tbl_mgr::Client::get_instance()->get_schema(table->db(), tid,
