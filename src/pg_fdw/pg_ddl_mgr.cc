...
static constexpr char DROP_DATABASE[] = "DROP DATABASE IF EXISTS {} WITH (FORCE)";

static constexpr char VERIFY_DB_EXISTS[] = "SELECT 1 FROM  pg_database WHERE datname = '{}'";
               ...
               const std::optional<std::string> &hostname)
{
    ...
    SPDLOG_DEBUG("FDW ID: {}, Host: {}, Port: {}, Username: {}, FDW Username: {}", _fdw_id,
                 _hostname, _port, _username, fdw_username);
...
PgDDLMgr::_internal_run()
{
    ...
    while (!_is_shutting_down()) {
        try {
            ...
            SPDLOG_DEBUG_MODULE(LOG_FDW, "Applying DDLs for db_id: {}, schema_xid: {}", db_id,
                                schema_xid);
            ...
            SPDLOG_DEBUG_MODULE(LOG_FDW, "Updating redis ddl @ schema XID: {}, db_id: {}",
                                schema_xid, db_id);
                          ...
                          const nlohmann::json &ddls)
{
    ...
    try {
        ...
        txn.push_back(fmt::format("ALTER SERVER {} OPTIONS (SET {} '{}')",
                                  SPRINGTAIL_FDW_SERVER_NAME, SPRINGTAIL_FDW_SCHEMA_XID_OPTION,
                                  schema_xid));
        ...
        std::shared_lock db_lock(_db_mutex);
        std::set_difference(schemas.begin(), schemas.end(), _db_schemas[db_id].begin(),
                            _db_schemas[db_id].end(),
                       ...
                       const std::set<std::string> &schemas)
{
    ...
    for (const auto &schema : schemas) {
        ...
        conn->exec(fmt::format(CREATE_SCHEMA_WITH_GRANTS, escaped_schema, escaped_schema,
                               _fdw_username, escaped_schema, _fdw_username, escaped_schema,
                               _fdw_username));
        conn->clear();
    }
                             ...
                             std::set<std::string> &schemas)
{
    ...
    if (action == "create") {
        ...
        for (const auto &col : ddl.at("columns")) {
            columns.push_back(
                {col.at("name"), type_map.at(col.at("type").get<uint32_t>()), col.at("nullable")});
        }
        ...
        std::string rename =
            fmt::format("ALTER FOREIGN TABLE {}.{} RENAME TO {};",
        ...
        if (ddl.at("schema").get<std::string>() != ddl.at("old_schema").get<std::string>()) {
            return rename +
                   fmt::format("ALTER FOREIGN TABLE {}.{} SET SCHEMA {};",
                               conn->escape_identifier(ddl.at("old_schema").get<std::string>()),
                               conn->escape_identifier(ddl.at("old_table").get<std::string>()),
        ...
        std::string null_constraint = col.at("nullable").get<bool>() ? "NULL" : "NOT NULL";
        if (col.contains("default")) {
            constraints =
                fmt::format("{} {}", null_constraint, col.at("default").get<std::string>());
        } else {
        ...
        return fmt::format("ALTER FOREIGN TABLE {}.{} ADD COLUMN {} {} {};",
                           ...
                           type_map.at(type_oid), constraints);
    }
...
PgDDLMgr::_query_type_names(LibPqConnectionPtr conn, std::set<uint32_t> pg_types)
{
    ...
    std::string &&query =
        fmt::format("SELECT oid, typname FROM pg_type WHERE oid IN ({})",
                             ...
                             std::vector<std::tuple<std::string, std::string, bool>> &columns)
{
    ...
    std::string create =
        fmt::format("CREATE FOREIGN TABLE {}.{} (\n", conn->escape_identifier(schema),
    ...
    for (int i = 0; i < columns.size(); i++) {
        ...
        std::string column =
            fmt::format("{} {} {} {}", conn->escape_identifier(column_name), type_name,
                        nullable ? "" : "NOT NULL", (i == columns.size() - 1) ? "" : ",");
...
void
PgDDLMgr::_create_schemas(LibPqConnectionPtr conn, const uint64_t db_id, const std::string &db_name)
{
    RedisDDL redis_ddl;
    ...
    conn->exec(fmt::format(
        "CREATE SERVER {} FOREIGN DATA WRAPPER {} OPTIONS (id '{}', db_id '{}', db_name '{}', "
        "schema_xid '{}')",
    ...
    for (const auto &schema : schemas) {
        ...
        conn->exec(fmt::format("IMPORT FOREIGN SCHEMA {} FROM SERVER {} INTO {}", escaped_schema,
                               SPRINGTAIL_FDW_SERVER_NAME, escaped_schema));
        conn->clear();
        ...
        conn->clear();
        conn->exec(fmt::format("GRANT SELECT ON ALL TABLES IN SCHEMA {} TO {}", escaped_schema,
                               _fdw_username));
        conn->clear();
        conn->exec(fmt::format("GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA {} TO {}",
...
PgDDLMgr::_handle_replicated_dbs_change(const std::string &msg)
{
    ...
    switch (action) {
        ...
        case redis::db_state_change::DB_ACTION_REMOVE:
            ...
            break;
        default:
            SPDLOG_DEBUG_MODULE(LOG_FDW, "Unsupported action: {}",
                                redis::db_state_change::db_action_to_name[action]);
    }
}
