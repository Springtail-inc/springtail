...
namespace springtail::pg_fdw {
    ...
    std::string PgDDLMgr::_get_type_name(int32_t pg_type,
    ...
    std::string
    PgDDLMgr::_get_create_partitioned_table_query(std::string schema,
                                        ...
                                        std::string partition_bound)
    {
        ...
        std::string create = fmt::format("CREATE TABLE {}.{} \n",
                                         ...
                                         conn->escape_identifier(table_name));
        ...
        if (parent_table_name.empty()) {
            ...
            for (int i = 0; i < columns.size(); i++) {
                ...
                std::string column = fmt::format("  {} ", conn->escape_identifier(column_name.c_str()));
            ...
            create += fmt::format("\nPARTITION OF {}.{}", conn->escape_identifier(schema), conn->escape_identifier(parent_table_name));
            create += fmt::format("\n{}", partition_bound);
            create += fmt::format("\nPARTITION BY {}", partition_key);
        }
    ...
    void
    PgDDLMgr::_process_table(LibPqConnectionPtr conn,
    ...
    {
        const auto &partition_info = table_partition_map.at(current_tid);
        ...
        if ( parent_table_id == 0 && !partition_key.empty()) {
            ...
            LOG_DEBUG(LOG_FDW, "Parent partitioned table SQL: {}", sql);
            conn->exec(sql);
        } else if (!partition_bound.empty() && !partition_key.empty()) {
            std::string sql = _get_create_partitioned_table_query(namespace_name, current_table, conn, columns, parent_table_name, partition_key, partition_bound);
            LOG_DEBUG(LOG_FDW, "Non-leaf partitioned table SQL: {}", sql);
            conn->exec(sql);
        } else {
            LOG_DEBUG(LOG_FDW, "Leaf partitioned table: {}", current_table);
        }
    }

    void
    PgDDLMgr::_create_partition_tables(uint64_t db_id,
                             uint64_t schema_xid,
                             ...
                             LibPqConnectionPtr conn)
    {
        ...
        for (auto row : (*table)) {
            ...
            if (table_ns_id != namespace_id) {
                LOG_DEBUG(LOG_FDW, "Skipping row due to namespace mismatch {}, {}", table_ns_id, namespace_id);
                continue;
            }
        ...
        for (auto &partition_info : table_partition_map) {
            ...
            std::string partition_key = partition_info.second.partition_key;
            std::string partition_bound = partition_info.second.partition_bound;

            auto parent_table = tid_map.find(parent_table_id);
            if (parent_table == tid_map.end()) {
                LOG_WARN("Parent table {} not found for partition {}", parent_table_id, partition_info.first);
                continue;
            }
        ...
        for (auto row : (*table)) {
            ...
            if (tid != current_tid) {
                if (!current_table.empty()) {
                    _process_table(conn, namespace_name, current_table, current_tid, columns, table_partition_map);
                }
            ...
            columns.emplace_back(column_name, _get_type_name(pg_type, user_types), nullable);
        }
                    ...
                    const std::string &db_name)
    {
        ...
        for (const auto &schema: schemas) {
            ...
            _create_partition_tables(db_id, xid, schema.second, user_types, conn);
