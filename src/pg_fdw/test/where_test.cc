...
protected:
    ...
    static void SetUpTestSuite()
    {
        ...
        _columns = {
            {"col1", static_cast<uint8_t>(SchemaType::INT32), INT4OID, std::nullopt, 1, 0, false,
             true, false},
            {"col2", static_cast<uint8_t>(SchemaType::INT32), INT4OID, std::nullopt, 2, 1, false,
             true, false},
            {"col3", static_cast<uint8_t>(SchemaType::INT32), INT4OID, std::nullopt, 3, 2, false,
             true, false},
            {"col4", static_cast<uint8_t>(SchemaType::INT32), INT4OID, std::nullopt, 4, 0, false,
             false, false},
            {"col5", static_cast<uint8_t>(SchemaType::INT32), INT4OID, std::nullopt, 5, 0, false,
             false, false},
        };

        _data = {{1, 1, 1, 1, 1}, {1, 2, 2, 2, 2}, {1, 3, 1, 1, 1}, {1, 3, 2, 2, 3},
                 {1, 5, 1, 1, 1}, {1, 5, 2, 2, 2}, {2, 1, 1, 1, 1}, {2, 2, 2, 2, 2},
                 {2, 3, 1, 1, 1}, {2, 3, 2, 2, 2}, {2, 5, 1, 1, 1}, {2, 5, 2, 2, 2},
                 {3, 1, 1, 1, 1}, {3, 2, 2, 2, 2}, {3, 3, 1, 1, 1}, {3, 3, 2, 2, 2},
                 {5, 2, 1, 3, 3}, {5, 4, 2, 3, 3}};
        ...
        auto mtable = TableMgr::get_instance()->get_mutable_table(_db_id, _tid, access_xid,
                                                                  target_xid, false);
    ...
    static void _create_table(uint64_t db_id, uint64_t table_id, uint64_t xid)
    {
        // create a table
    ...
    static void _create_index(uint64_t db_id, uint64_t table_id, uint64_t xid)
    {
        ...
        sys_tbl_mgr::Client::get_instance()->create_index(db_id, xid_lsn, msg,
                                                          sys_tbl::IndexNames::State::READY);
    }

    static void _drop_index(uint64_t db_id, uint32_t index_id, uint64_t xid)
    {
        PgMsgDropIndex msg;
    ...
    static std::shared_ptr<Tuple> _create_value(const std::vector<int32_t> &data)
    {
    ...
    static void _populate_table(MutableTablePtr mtable, uint64_t xid)
    {
    ...
    List *_add_sortgroup(int attnum, bool reversed, List *sort_list = nullptr)
    {
    ...
    List *_add_qual(int attno, QualOpName op, int32_t val, List *qual_list = nullptr)
    {
    ...
    std::vector<std::vector<int32_t>> _filter_data(List *qual_list)
    {
    ...
    void _dump_filtered_data(List *quals, const std::vector<std::vector<int32_t>> &filtered_data)
    {
        ListCell *lc;
        foreach (lc, quals) {
            ConstQualPtr qual = static_cast<ConstQualPtr>(lfirst(lc));
            std::cout << "Column: " << qual->base.varattno << " " << qual->base.opname
                      << " Value: " << DatumGetInt32(qual->value) << std::endl;
        }
    ...
    void _run_scan(List *qual_list,
                   const std::vector<std::vector<int32_t>> &filtered_data,
...
TEST_F(FDWWhere_Test, Test_Secondary)
{
    ...
    // sort by col4
    std::ranges::sort(sorted_data, [](auto const &a, auto const &b) { return a[3] < b[3]; });
