...
Properties::_create_redis_client()
{
    ...
    if (_json.contains(REDIS_CONFIG) == false) {
        throw Error(
            "Error missing redis config in environment\nTry setting SPRINGTAIL_PROPERTIES_FILE to "
            "settings.json");
    }

    nlohmann::json redis_config = _json[REDIS_CONFIG];
    if (redis_config.contains("host") == false || redis_config["host"].is_null() ||
        redis_config.contains("port") == false || redis_config["port"].is_null() ||
        redis_config.contains("config_db") == false || redis_config["config_db"].is_null() ||
        redis_config.contains("db") == false || redis_config["db"].is_null()) {
        throw Error(
            "Error missing redis config in environment\nTry setting SPRINGTAIL_PROPERTIES_FILE to "
            "settings.json");
...
Properties::_read_redis_properties()
{
    ...
    // read the system properties from redis
    std::optional<std::string> system_value =
        _redis_config_client->hget(db_instance_key, system_key);
...
Properties::_load_redis(const std::string &config_file)
{
    ...
    _redis_config_client->hset(db_instance_key, "primary_db",
                               db_instance_json["primary_db"].dump());

    // Set the hostnames for ingestion and proxy instances
    _redis_config_client->hset(db_instance_key, "hostname:ingestion",
                               db_instance_json["hostname:ingestion"].get<std::string>());
    _redis_config_client->hset(db_instance_key, "hostname:proxy",
                               db_instance_json["hostname:proxy"].get<std::string>());
...
Properties::set_env_from_file(const char *config_file)
{
    ...
    for (auto &variable : environment::Variables) {
        ...
        if (system_json.contains(json_obj_name) &&
            system_json[json_obj_name].contains(json_key_name)) {
...
Properties::get_db_config(uint64_t db_id)
{
    ...
    std::optional<std::string> db_config_str =
        redis_client->hget(db_config_key, std::to_string(db_id));
...
Properties::get_db_state(uint64_t db_id)
{
    ...
    std::optional<std::string> db_state_str =
        redis_client->hget(db_instance_state_hash, std::to_string(db_id));
...
Properties::set_db_state(uint64_t db_id, const std::string &state)
{
    ...
    // publish the state
    redis_client->publish(fmt::format(redis::PUBSUB_DB_STATE_CHANGES, db_instance_id),
                          fmt::format("{}:{}", db_id, state));
}
...
void
Properties::get_primary_db_config(std::string &host,
                                  int &port,
                                  std::string &user,
                                  std::string &password)
{
    ...
    if (optional_host.has_value() && optional_port.has_value() && optional_user.has_value() &&
        optional_password.has_value()) {
        ...
        SPDLOG_ERROR(
            "Could not find the value for primary database either host, port, user, or password");
        throw Error();
    }
}
...
Properties::_get_ingestion_hostname()
{
    ...
    RedisClientPtr redis_client = _get_redis_client();
    std::optional<std::string> hostname = redis_client->hget(
        std::format(redis::DB_INSTANCE_CONFIG, db_instance_id), "hostname:ingestion");
...
Properties::get_pid_path()
{
    ...
    std::string pid_path =
        Json::get_or<std::string>(props, Properties::PID_PATH, "/var/springtail/pids");
    return pid_path;
}
...
Properties::init(bool load_redis)
{
    if (_instance == nullptr) {
        // once init
        std::call_once(_init_flag, [load_redis]() { _instance = new Properties(load_redis); });
    }
}
}  // namespace springtail
