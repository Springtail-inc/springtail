...
DNSResolver::DNSResolver()
{
    ...
    if (!ares_threadsafety()) {
        SPDLOG_ERROR("c-ares is not thread safe");
...
DNSResolver::set_ttl_secs(int ttl)
{
    ...
    if (ares_init_options(&_channel, &options, optmask) != ARES_SUCCESS) {
        SPDLOG_ERROR("Failed to set DNS cache TTL");
...
DNSResolver::resolve(const std::string &hostname)
{
    ...
    ares_getaddrinfo(_channel, hostname.c_str(), nullptr, &hints, _addrinfo_cb, &request);
...
void
DNSResolver::_addrinfo_cb(void *arg, int status, int timeouts, ares_addrinfo *result)
{
    ...
    if (status != ARES_SUCCESS || result == nullptr) {
        SPDLOG_ERROR("Failed to resolve hostname: status: {}", status);
        if (result != nullptr) {
