...
PgLogMgr::_startup_running()
{
    ...
    std::filesystem::path latest_log =
        fs::find_latest_modified_file(_repl_log_path, LOG_PREFIX_REPL, LOG_SUFFIX);
    if (!latest_log.empty()) {
    ...
    while (true) {
        ...
        for (auto &xact : committed_xacts) {
            ...
            SPDLOG_DEBUG_MODULE(LOG_PG_LOG_MGR, "Replaying xact to redis: xid={}, type={}",
                                xact->springtail_xid, xact->type);
        }
    ...
    if (last_allocated_xid > current_xid) {
        SPDLOG_DEBUG_MODULE(LOG_PG_LOG_MGR, "Updating next xid: {} -> {}", current_xid,
                            last_allocated_xid + 1);
    ...
    if (last_xact != nullptr) {
        ...
        std::filesystem::path next_log =
            fs::get_next_file(last_xact->commit_path, LOG_PREFIX_REPL, LOG_SUFFIX);
...
PgLogMgr::_handle_external_state_change(const redis::db_state_change::DBState new_state)
{
    ...
    if (new_state == redis::db_state_change::DB_STATE_RUNNING) {
        ...
        if (internal_state == STATE_SYNC_STALL || internal_state == STATE_STARTUP_SYNC) {
        ...
        SPDLOG_DEBUG_MODULE(LOG_PG_LOG_MGR, "Current state is running: {}",
                            (_internal_state.get() == STATE_RUNNING));
    }
}
...
PgLogMgr::_redis_pubsub_thread()
{
    ...
    subscriber->on_message(
        [this, state_change_channel](const std::string &channel, const std::string &msg) {
            ...
            SPDLOG_DEBUG_MODULE(LOG_PG_LOG_MGR, "Received state change: {}",
                                redis::db_state_change::db_state_to_name[state]);
...
PgLogMgr::_copy_thread()
{
    ...
    while (!_shutdown) {
        ...
        StringPtr table_id_ptr =
            _redis_sync_queue.pop(REDIS_WORKER_ID, constant::COORDINATOR_KEEP_ALIVE_TIMEOUT);
...
PgLogMgr::_process_copy_results(const std::vector<PgCopyResultPtr> &res)
{
    ...
    for (const auto &r : res) {
        ...
        SPDLOG_DEBUG_MODULE(LOG_PG_LOG_MGR, "Recording table sync msgs: target_xid={}",
                            r->target_xid);
        PgXactMsg redis_xact(_db_id, r);

        bool sync_start = SyncTracker::get_instance()->add_sync(
            std::get<pg_log_mgr::PgXactMsg::TableSyncMsg>(redis_xact.msg));

        // notify the Committer to stop committing XIDs
        if (sync_start) {
            RedisQueue<gc::XidReady> committer_queue(
                fmt::format(redis::QUEUE_GC_XID_READY, Properties::get_db_instance_id()));
...
PgLogMgr::_log_writer_thread()
{
    ...
    while (!_shutdown) {
        ...
        SPDLOG_DEBUG_MODULE(LOG_PG_LOG_MGR,
                            "Recevied data: length={}, msg_length={}, msg_offset={}", data.length,
...
PgLogMgr::_log_reader_thread()
{
    ...
    while (!_shutdown) {
        ...
        PgLogQueueEntryPtr log_entry =
            this->_logger_queue.pop(constant::COORDINATOR_KEEP_ALIVE_TIMEOUT);
        ...
        SPDLOG_DEBUG_MODULE(LOG_PG_LOG_MGR,
                            "Got log entry: path={}, start_offset={}, num_messages={}",
        ...
        SPDLOG_DEBUG_MODULE(LOG_PG_LOG_MGR,
                            "Processing log entry: path={}, start_offset={}, num_messages={}",
                            log_entry->path, log_entry->start_offset, log_entry->num_messages);
...
PgLogMgr::_create_xact_logger()
{
    ...
    std::filesystem::path file =
        fs::find_latest_modified_file(_xact_log_path, LOG_PREFIX_XACT, LOG_SUFFIX);
    if (file.empty()) {
...
PgLogMgr::_process_xact(const PgTransactionPtr xact)
{
    ...
    if (xact->xact_lsn <= _last_pushed_lsn) {
        SPDLOG_WARN("Skipping xact (already seen): xact_lsn={}, last_pushed_lsn={}", xact->xact_lsn,
                    _last_pushed_lsn);
        return;
    }
