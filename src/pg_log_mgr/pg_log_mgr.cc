...
namespace springtail::pg_log_mgr {
                              ...
                              PgCopyData &data)
    {
        ...
        LOG_DEBUG(LOG_PG_LOG_MGR_DATA, "Received data: length={}, msg_length={}, msg_offset={}, start_lsn={}, end_lsn={}",
        ...
        std::function<void(uint64_t, uint64_t, const std::filesystem::path &)> queue_append_func)
    {
        ...
        // isn't a WAL message, e.g., if it is generated by pgoutput
        // e.g., a RELATION, STREAM_START, STREAM_STOP message

        // check if we need to do a rotation, check before logging the data
        // as we need to rotate on LSN boundaries and can't do that if we
        // have already written the data
        // check this is a new message and not a continuation
        if (current_log_offset > _log_size_rollover_threshold &&
            data.msg_offset == 0 &&
            data.starting_lsn != INVALID_LSN &&
            data.starting_lsn != _current_lsn) {
            LOG_DEBUG(LOG_PG_LOG_MGR, "Rollover log before write: path={}, offset={}",
                      logger->filename(), logger->offset());

            // see if we have unpushed messages to queue
            if (current_log_offset > _msg_log_start_offset) {
                // queue messages in this LSN
                queue_append_func(_msg_log_start_offset, current_log_offset, logger->filename());
            }
        ...
        // moving to a new LSN, so queue up any messages in the previous LSN
        if (data.starting_lsn != INVALID_LSN &&
            _current_lsn != data.starting_lsn) {
            LOG_DEBUG(LOG_PG_LOG_MGR_DATA, "New LSN: current_lsn={}, new_lsn={}, prev_msg_type={}",
                      _current_lsn, data.starting_lsn, _current_msg_type);

            // check for a pending message
            if (current_log_offset > _msg_log_start_offset) {
                LOG_DEBUG(LOG_PG_LOG_MGR, "New LSN, queueing messages: path={}, start_offset={}, end_offset={}",
                          logger->filename(), _msg_log_start_offset, current_log_offset);

                // queue messages already written messages
                queue_append_func(_msg_log_start_offset, current_log_offset, logger->filename());

                // update start offset for next message
                _msg_log_start_offset = current_log_offset;
            }

            // update last lsn
            _current_lsn = data.starting_lsn;
            _expect_lsn_change = false;
        }

        if (_expect_lsn_change && data.starting_lsn != INVALID_LSN) {
            LOG_WARN("Expecting LSN change but didn't get one: current_lsn={}, new_lsn={}",
                     _current_lsn, data.starting_lsn);
        }
        ...
        // if we got here, we have a complete xlog message, but
        // it might be split across LSN boundaries, so we need
        // to check if it is a commit or an abort message, if it is
        // then the next message will be on a new LSN
        if (_current_msg_type == pg_msg::MSG_COMMIT ||
            _current_msg_type == pg_msg::MSG_STREAM_COMMIT ||
            _current_msg_type == pg_msg::MSG_STREAM_ABORT) {
            ...
            LOG_DEBUG(LOG_PG_LOG_MGR_DATA, "Complete commit/abort message: path={}, start_offset={}, end_offset={}, msg_type={}",
                      logger->filename(), _msg_log_start_offset, end_offset, (char)_current_msg_type);

            _expect_lsn_change = true;
        }
