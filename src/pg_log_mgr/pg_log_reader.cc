        ...
        if (entry.extent == nullptr) {
            if (entry.schema == nullptr) {
                ...
                if (entry.table_schema == nullptr) {
                    entry.table_schema = TableMgr::get_instance()->get_extent_schema(_db, tid, xidlsn, true, false);
                }
                entry.update_schema();
            }
                                 ...
                                 const PgMsgTruncate &msg)
    {
        ...
        for (auto tid : msg.rel_ids) {
            ...
            if (entry.schema == nullptr) {
                ...
                entry.table_schema = sync_skip.schema();
                if (entry.table_schema == nullptr) {
                    entry.table_schema = TableMgr::get_instance()->get_extent_schema(_db, tid, current, true, false);
                }
                entry.update_schema();
            }
