...
void
PgLogReader::Batch::truncate(uint64_t current_xid, const PgMsgTruncate &msg)
{
    auto scope = tracing::tracer("PgLogReader")->WithActiveSpan(_span);
...
PgLogReader::Batch::schema_change(int32_t tid, int32_t pg_xid, PgMsgPtr msg)
{
    ...
    if (msg->msg_type == PgMsgEnum::CREATE_TABLE || msg->msg_type == PgMsgEnum::ALTER_TABLE) {
        ...
        for (auto column : table.columns) {
            columns.emplace_back(SchemaColumn{column.column_name, column.position,
...
PgLogReader::Batch::_start_subtxn(int32_t pg_xid)
{
    ...
    if (_cur_txn != nullptr) {
        for (auto &entry : _cur_txn->table_map) {
            ...
            WriteCacheFuncImpl::add_extent(_db, entry.first, _cur_txn->pg_xid, table.start_lsn,
                                           table.extent);
...
void
PgLogReader::Batch::_apply_schema_changes(const LsnChangeMap &change_map, uint64_t xid)
{
...
void
PgLogReader::Batch::_apply_schema_change(PgMsgPtr change, const XidLsn &xidlsn)
{
    RedisDDL redis_ddl;
    ...
    switch (change->msg_type) {
        ...
        case PgMsgEnum::ALTER_TABLE: {
            ...
            if (action.get<std::string>() == "resync") {
                ...
                auto key =
                    fmt::format(redis::QUEUE_SYNC_TABLES, Properties::get_db_instance_id(), _db);
        ...
        case PgMsgEnum::CREATE_INDEX: {
            ...
            std::string &&ddl_stmt =
                client->create_index(_db, xidlsn, index_msg, sys_tbl::IndexNames::State::READY);
...
void
PgLogReader::process_log(const std::filesystem::path &path, uint64_t start_offset, int num_messages)
{
    // init stream reader
...
PgLogReader::_process_msg(PgMsgPtr msg)
{
    ...
    switch (msg->msg_type) {
        ...
        case PgMsgEnum::TRUNCATE: {
            _current_batch->truncate(this->get_current_xid(), std::get<PgMsgTruncate>(msg->msg));
            break;
        }
        case PgMsgEnum::CREATE_TABLE:
        case PgMsgEnum::ALTER_TABLE: {
...
void
PgLogReader::_check_sync_commit(uint64_t db_id, int32_t pg_xid, uint64_t xid)
{
...
PgLogReader::_process_stream_start(const PgMsgStreamStart &start_msg)
{
    ...
    PgTransactionPtr stream_xact =
        std::make_shared<PgTransaction>(PgTransaction::TYPE_STREAM_START);
...
PgLogReader::_process_stream_commit(const PgMsgStreamCommit &commit_msg)
{
    ...
    if (itr == _xact_map.end()) {
        ...
        SPDLOG_WARN("No matching xact for stream commit: xid={}, xact_lsn={}", commit_msg.xid,
                    commit_msg.xact_lsn);
        return;
    }
...
PgLogReader::_process_stream_abort(const PgMsgStreamAbort &abort_msg)
{
    ...
    if (itr == _xact_map.end()) {
        // no start streaming xact found...
        SPDLOG_WARN("No matching xact for stream abort: xid={}, xact_lsn={}", abort_msg.xid,
                    abort_msg.abort_lsn);
        return;
    }
