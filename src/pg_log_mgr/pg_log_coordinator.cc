...
PgLogCoordinator::init()
{
    ...
    _config_sub_thread.add_subscriber(
        db_change_channel, [this]() { this->_init_db_change_subscriber(); },
        [this](const std::string &msg) { _handle_db_changes(msg); });

    _config_sub_thread.start();

    // create a thread for the write cache
    _write_cache_thread = std::thread([]() { WriteCacheServer::get_instance()->startup(); });
}
...
PgLogCoordinator::_add_database(uint64_t db_id)
{
    ...
    std::filesystem::path repl_log_path =
        Properties::make_absolute_path(_repl_log) / std::to_string(db_id);
    std::filesystem::path xact_log_path =
        Properties::make_absolute_path(_trans_log) / std::to_string(db_id);
    ...
    // create log mgr
    PgLogMgrPtr log_mgr =
        std::make_shared<PgLogMgr>(db_id, repl_log_path, xact_log_path, _host, db_name, _user_name,
                                   _password, pub_name, slot_name, _port);
...
PgLogCoordinator::_handle_db_changes(const std::string &msg)
{
    ...
    switch (action) {
        ...
        case redis::db_state_change::DB_ACTION_REMOVE:
            ...
            break;
        default:
            SPDLOG_DEBUG_MODULE(LOG_PG_LOG_MGR, "Unsupported action: {}",
