...
bool
SyncTracker::mark_resync(uint64_t db_id, uint64_t table_id)
{
...
SyncTracker::check_commit(uint64_t db_id, uint32_t pg_xid)
{
    ...
    // We pass the target XID to the committer to be used if the completed XID is not ahead of the
    // committed XID.
...
void
SyncTracker::clear_tables(uint64_t db_id, const gc::XidReady &commit_msg)
{
...
bool
SyncTracker::should_skip(uint64_t db_id, uint64_t table_id, uint32_t pg_xid) const
{
