...
Partition::_write_committed_xids()
{
    ...
    std::copy_n(reinterpret_cast<const char *>(&num_entries), sizeof(num_entries),
                buffer + sizeof(MAGIC_HDR));
...
Partition::_read_committed_xids()
{
    ...
    for (int i = 0; i < num_entries; i++) {
        ...
        SPDLOG_DEBUG_MODULE(LOG_XID_MGR, "Partition: read committed xids: db_id={}, xid={}", db_id,
                            xid);
    }
}

void
Partition::_add_history(uint64_t db_id, uint64_t xid)
{
...
void
Partition::record_ddl_change(uint64_t db_id, uint64_t xid)
{
    std::unique_lock lock(_map_mutex);
    _add_history(db_id, xid);
}
