...
{
    SPDLOG_DEBUG_MODULE(LOG_WRITE_CACHE_SERVER, "{}, {}, {}", target_xid, old_eid,
                        new_eids.front());
...
void
TableExtentMapper::set_lookup(uint64_t target_xid, uint64_t extent_id)
{
...
std::vector<uint64_t>
TableExtentMapper::forward_map(uint64_t target_xid, uint64_t extent_id)
{
    SPDLOG_DEBUG("{}, {}", target_xid, extent_id);
...
std::vector<uint64_t>
TableExtentMapper::reverse_map(uint64_t access_xid, uint64_t target_xid, uint64_t extent_id)
{
    ...
    if (it != _reverse_map.end()) {
        ...
        for (auto eid : it->second) {
            ...
            if (lookup_i != _lookup_map.end() && lookup_i->second.latest_xid > access_xid &&
        ...
        for (auto &entry : f_it->second) {
            if (entry.xid < target_xid) {
                for (auto &eid : entry.eids) {
                    ...
                    if (l != _lookup_map.end() && l->second.latest_xid > access_xid &&
        ...
        for (auto eid : it->second) {
            ...
            if (l->second.latest_xid <= xid) {
                ...
                for (auto &entry : f->second) {
                    SPDLOG_DEBUG_MODULE(LOG_WRITE_CACHE_SERVER, "Try clear reverse xid@{}",
                                        entry.xid);
                    ...
                    for (auto new_eid : entry.eids) {
                        SPDLOG_DEBUG_MODULE(LOG_WRITE_CACHE_SERVER, "Clear reverse entry {}",
                                            new_eid);
                        ...
                        if (r->second.empty()) {
                            SPDLOG_DEBUG_MODULE(LOG_WRITE_CACHE_SERVER, "Clear empty reverse {}",
                                                r->first);
...
void
ExtentMapper::set_lookup(uint64_t tid, uint64_t target_xid, uint64_t extent_id)
{
    Mapper mapper(this, tid, true);
...
std::vector<uint64_t>
ExtentMapper::forward_map(uint64_t tid, uint64_t target_xid, uint64_t extent_id)
{
...
void
ExtentMapper::expire(uint64_t tid, uint64_t commit_xid)
{
    Mapper mapper(this, tid, true);
