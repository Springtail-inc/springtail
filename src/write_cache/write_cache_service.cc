...
void
ThriftWriteCacheService::get_extents(thrift::write_cache::GetExtentsResponse &_return,
                                     const thrift::write_cache::GetExtentsRequest &request)
{
    ...
    std::vector<WriteCacheIndexExtentPtr> extents =
        index->get_extents(request.table_id, request.xid, request.count, cursor);
...
{
    ExtentMapper *mapper = ExtentMapper::get_instance(request.db_id);
    auto &&response = mapper->forward_map(request.table_id, request.target_xid, request.extent_id);
...
{
    ExtentMapper *mapper = ExtentMapper::get_instance(request.db_id);
    auto &&response = mapper->reverse_map(request.table_id, request.access_xid, request.target_xid,
                                          request.extent_id);
