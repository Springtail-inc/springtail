...
public:
    ...
    /** add row constructor */
    WriteCacheIndexTestRequest(WriteCacheTableSetPtr ts,
                               uint64_t tid,
                               uint64_t eid,
    ...
    WriteCacheIndexTestRequest(
        WriteCacheTableSetPtr ts, uint64_t start_xid, uint64_t end_xid, uint64_t tid, Type type)
     ...
     */
    void operator()() { _process_request(); }
...
protected:
    ...
    WriteCacheIndexTestRequestPtr _make_row_request(
        uint64_t tid, uint64_t eid, uint64_t xid, uint64_t xid_seq, int rid_start, int count);

    /** Helper to construct a vector of rows */
    void _make_rows(uint64_t eid,
                    uint64_t xid,
                    uint64_t xid_seq,
                    int rid_start,
                    int count,
    ...
    WriteCacheIndexTestRequestPtr _make_eviction_request(uint64_t tid,
                                                         uint64_t start_xid,
                                                         uint64_t end_xid);

    /** Helper to create table change addition request */
    WriteCacheIndexTestRequestPtr _make_table_change_request(uint64_t tid,
                                                             uint64_t xid,
                                                             uint64_t xid_seq);

    /** Helper to create table change eviction request */
    WriteCacheIndexTestRequestPtr _make_table_change_eviction_request(uint64_t tid,
                                                                      uint64_t start_xid,
                                                                      uint64_t end_xid);
};
...
template <class T>
static void
vec_eq(const std::vector<T> &lhs, const std::vector<T> &rhs)
{
...
template <class T>
static void
vec_eq(const std::vector<T> &lhs, const std::vector<T> &rhs, bool (*cmp_fn)(const T &, const T &))
{
    ASSERT_EQ(lhs.size(), rhs.size());
...
static bool
row_cmp_fn(const WriteCacheIndexRowPtr &lhs, const WriteCacheIndexRowPtr &rhs)
{
    return (lhs->eid == rhs->eid && lhs->xid == rhs->xid && lhs->xid_seq == rhs->xid_seq &&
            lhs->pkey == rhs->pkey);
}

/** comparator for table change vectors */
static bool
table_change_cmp_fn(const WriteCacheIndexTableChangePtr &lhs,
                    const WriteCacheIndexTableChangePtr &rhs)
{
...
WriteCacheIndexTestRequest::_process_request()
{
    switch (_type) {
        ...
        case Type::ADD_TABLE_CHANGE: {
            ...
            WriteCacheIndexTableChangePtr req = std::make_shared<WriteCacheIndexTableChange>(
                _tid, _start_xid, _xid_seq,
...
WriteCacheIndexTest::SetUp()
{
    ...
    std::cout << "Init allocated bytes="
              << TrackingAllocatorStats<TrackingAllocatorTags::TAG_WRITE_CACHE>::get_instance()
                     ->get_allocated_bytes()
              << std::endl;
    _ts->dump();
}
...
{
    std::cout << "Shutdown allocated bytes="
              << TrackingAllocatorStats<TrackingAllocatorTags::TAG_WRITE_CACHE>::get_instance()
                     ->get_allocated_bytes()
              << std::endl;
    _ts->dump();
...
void
WriteCacheIndexTest::_make_rows(uint64_t eid,
                                uint64_t xid,
                                uint64_t xid_seq,
                                int rid_start,
                                int count,
                                std::vector<WriteCacheIndexRowPtr> &rows)
{
    for (int i = 0; i < count; i++) {
        // pkey, xid,
        rows.push_back(
            std::make_shared<WriteCacheIndexRow>("data", fmt::format("{}", rid_start + i), eid, xid,
                                                 xid_seq, WriteCacheIndexRow::RowOp::INSERT));
    }
}

WriteCacheIndexTestRequestPtr
WriteCacheIndexTest::_make_row_request(
    uint64_t tid, uint64_t eid, uint64_t xid, uint64_t xid_seq, int rid_start, int count)
{
    std::vector<WriteCacheIndexRowPtr> rows;
...
{
    return std::make_shared<WriteCacheIndexTestRequest>(
        _ts, start_xid, end_xid, tid, WriteCacheIndexTestRequest::Type::EVICT_TABLE);
}
...
WriteCacheIndexTestRequestPtr
WriteCacheIndexTest::_make_table_change_eviction_request(uint64_t tid,
                                                         uint64_t start_xid,
                                                         uint64_t end_xid)
{
    return std::make_shared<WriteCacheIndexTestRequest>(
        _ts, start_xid, end_xid, tid, WriteCacheIndexTestRequest::Type::EVICT_TABLE_CHANGE);
}
...
TEST_F(WriteCacheIndexTest, Simple_Test)
{
    ...
    WriteCacheIndexRowPtr row = std::make_shared<WriteCacheIndexRow>(
        "data", "data", 0, 11, 0, WriteCacheIndexRow::RowOp::INSERT);
    WriteCacheIndex idx{};
...
TEST_F(WriteCacheIndexTest, Threaded_Test)
{
    ...
    tester.set_verify([this]() {
        ...
        std::cout << "Phase 1 allocated bytes="
                  << TrackingAllocatorStats<TrackingAllocatorTags::TAG_WRITE_CACHE>::get_instance()
                         ->get_allocated_bytes()
                  << std::endl;
    });
    ...
    tester.set_verify([this]() {
        ...
        expected.push_back(std::make_shared<WriteCacheIndexTableChange>(
            1, 2, 0, WriteCacheIndexTableChange::TableChangeOp::SCHEMA_CHANGE));
        expected.push_back(std::make_shared<WriteCacheIndexTableChange>(
            1, 2, 1, WriteCacheIndexTableChange::TableChangeOp::SCHEMA_CHANGE));
        expected.push_back(std::make_shared<WriteCacheIndexTableChange>(
            1, 3, 0, WriteCacheIndexTableChange::TableChangeOp::SCHEMA_CHANGE));
        expected.push_back(std::make_shared<WriteCacheIndexTableChange>(
            1, 3, 1, WriteCacheIndexTableChange::TableChangeOp::SCHEMA_CHANGE));
    ...
    tester.set_verify([this]() {
        ...
        std::vector<WriteCacheIndexTableChangePtr> expected;
        expected.push_back(std::make_shared<WriteCacheIndexTableChange>(
            1, 1, 0, WriteCacheIndexTableChange::TableChangeOp::SCHEMA_CHANGE));
        expected.push_back(std::make_shared<WriteCacheIndexTableChange>(
            1, 1, 1, WriteCacheIndexTableChange::TableChangeOp::SCHEMA_CHANGE));
        ASSERT_NO_FATAL_FAILURE(vec_eq(changes, expected, table_change_cmp_fn));
