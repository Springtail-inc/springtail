...
protected:
    ...
    void _gc1(uint64_t tid, uint64_t xid)
    {
    ...
    std::vector<uint64_t> _gc2(uint64_t tid, uint64_t xid, int count = 1)
    {
        uint64_t lookup_eid;
        {
            boost::unique_lock lock(_write_cache_mutex);
        ...
        uint64_t update_eid = (m.empty()) ? lookup_eid : m.back();
...
TEST_F(ExtentMapper_Test, Threaded)
{
    ...
    // 1) acts like GC-1, performing set_lookup(); occassionally moves the lookup XID forward and
    // does expire() 2) acts like GC-2, performing forward_map() + add_mapping() 3) acts like query
    // nodes, performing reverse_map()
