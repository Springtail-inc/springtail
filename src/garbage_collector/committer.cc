...
Committer::run()
{
    ...
    while (!_shutdown) {
        ...
        if (result->type() == XidReady::Type::TABLE_SYNC_COMMIT ||
            ...
            SPDLOG_DEBUG_MODULE(LOG_GC,
                                "Handle a TABLE_SYNC_SWAP/COMMIT: {}, {}, completed xid @{}",
            ...
            std::string key =
                fmt::format(redis::HASH_SYNC_TABLE_OPS, Properties::get_db_instance_id(), db_id);
        ...
        for (auto const &ddl : ddls) {
            ...
            if (action == "create_index") {
                ...
                if (_index_exists(db_id, tid, index_id, xid)) {
                    ...
                    SPDLOG_DEBUG_MODULE(LOG_GC, "* Uncommitted index {}@{} -- {} {}", db_id, xid,
                                        tid, index_id);
                } else {
                    ...
                    XidLsn xid_c(xid);
                    sys_tbl_mgr::Client::get_instance()->create_index(
                        db_id, xid_c, msg, sys_tbl::IndexNames::State::READY);
...
void
Committer::_process_table(uint64_t db_id, uint64_t tid, uint64_t completed_xid, uint64_t xid)
{
