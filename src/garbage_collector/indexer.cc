...
void
Indexer::process_ddls(uint64_t db_id, uint64_t xid, nlohmann::json const& ddls)
{
...
void
Indexer::drop(uint64_t db_id, uint64_t index_id, uint64_t xid)
{
    std::scoped_lock g(_m);
...
void
Indexer::wait_for_completion(uint64_t db_id)
{
    auto find_work = [&]() {
        auto it =
            std::ranges::find_if(_work_set, [&](auto const& v) { return v.first.first == db_id; });
...
void
Indexer::wait_for_completion(uint64_t db_id, uint64_t tid)
{
    auto find_work = [&]() {
        auto it = std::ranges::find_if(_work_set, [&](auto const& v) {
...
void
Indexer::task(std::stop_token st)
{
...
void
Indexer::_drop(std::stop_token st, const Key& key, const IndexParams& idx)
{
...
Indexer::_build(std::stop_token st, const Key& key, const IndexParams& idx)
{
    ...
    for (auto row_i = table->begin(); row_i != table->end(); ++row_i) {
        ...
        (*value_fields)[1] =
            std::make_shared<ConstTypeField<uint32_t>>(static_cast<uint32_t>(row_id));
