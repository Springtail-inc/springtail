...
namespace springtail {
    ...
    uint64_t
    RedisDDL::min_fdw_pending_schema_xid(uint64_t db_id)
    {
        /*---------------- Check if DDL is pending to be applied in FDW --------------------------*/

        // get the set of FDWs
        std::vector<std::string> fdw_ids = Properties::get_fdw_ids();
        uint64_t db_instance_id = Properties::get_db_instance_id();
        uint64_t min_xid = UINT64_MAX;

        std::vector<std::string> keys;
        for (const std::string &fdw_id : fdw_ids) {
            keys.push_back(fmt::format(redis::QUEUE_DDL_FDW, db_instance_id, fdw_id));
        }

        const std::string db_prefix = std::to_string(db_id) + ":";

        auto ts = _redis->transaction(false, false);

        // Queue LRANGE all fdw pending and active queue entries
        for (const auto& base_key : keys) {
            ts.lrange(base_key, 0, -1);
            ts.lrange(base_key + ":active", 0, -1);
        }

        std::string precommit_key = fmt::format(redis::HASH_DDL_PRECOMMIT, db_instance_id);
        // Queue HKEYS for the precommit_key
        ts.hkeys(precommit_key);

        // Execute all at once
        auto replies = ts.exec();

        // Process committed entries separately
        size_t total_list_replies = keys.size() * 2;
        int index = 0;

        for (; index < total_list_replies; index++) {
            std::vector<std::string> committed_entries;
            replies.get(index, back_inserter(committed_entries));

            for (const auto& val : committed_entries) {
                auto parsed = nlohmann::json::parse(val);
                auto parsed_db_id = parsed["db_id"].get<uint64_t>();
                if (parsed_db_id == db_id) {
                    auto xid = parsed["xid"].get<uint64_t>();
                    if (xid < min_xid) {
                        min_xid = xid;
                    }
                }
        ...
        // Process precommitted_keys
        std::vector<std::string> precommitted_keys;
        replies.get(index, back_inserter(precommitted_keys));
        for (const auto&field : precommitted_keys) {
            if (field.rfind(db_prefix, 0) == 0) {  // starts with db_id:
                size_t pos = field.find(":");
                auto xid = std::stoull(field.substr(pos + 1));
                if (xid < min_xid) {
                    min_xid = xid;
                }
            }
