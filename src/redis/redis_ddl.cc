...
nlohmann::json
_get_ddls(RedisClient &redis, uint64_t db_id, uint64_t xid)
{
    std::string ddl_key = fmt::format(queue, Properties::get_db_instance_id(), db_id, xid);
...
void
_precommit(RedisClient &redis, uint64_t db_id, uint64_t xid, nlohmann::json ddls)
{
    nlohmann::json op;
    op["db_id"] = db_id;
    op["xid"] = xid;
    ...
    std::string precommit_key = fmt::format(hash_set, Properties::get_db_instance_id());
...
void
RedisDDL::add_ddl(uint64_t db_id, uint64_t xid, const std::string &ddl)
{
    std::string key =
        fmt::format(redis::QUEUE_DDL_XID, Properties::get_db_instance_id(), db_id, xid);
...
void
RedisDDL::add_index_ddl(uint64_t db_id, uint64_t xid, const std::string &ddl)
{
    std::string key =
        fmt::format(redis::QUEUE_INDEX_DDL_XID, Properties::get_db_instance_id(), db_id, xid);
...
nlohmann::json
RedisDDL::get_index_ddls_xid(uint64_t db_id, uint64_t xid)
{
    return _get_ddls<redis::QUEUE_INDEX_DDL_XID>(*_redis, db_id, xid);
}

nlohmann::json
RedisDDL::get_ddls_xid(uint64_t db_id, uint64_t xid)
{
    return _get_ddls<redis::QUEUE_DDL_XID>(*_redis, db_id, xid);
}

void
RedisDDL::clear_ddls_xid(uint64_t db_id, uint64_t xid)
{
    std::string ddl_key =
        fmt::format(redis::QUEUE_DDL_XID, Properties::get_db_instance_id(), db_id, xid);
    _redis->del(ddl_key);
...
void
RedisDDL::precommit_ddl(uint64_t db_id, uint64_t xid, nlohmann::json ddls)
{
    _precommit<redis::QUEUE_DDL_XID, redis::HASH_DDL_PRECOMMIT>(*_redis, db_id, xid, ddls);
}

void
RedisDDL::precommit_index_ddl(uint64_t db_id, uint64_t xid, nlohmann::json ddls)
{
    _precommit<redis::QUEUE_INDEX_DDL_XID, redis::HASH_DDL_INDEX_PRECOMMIT>(*_redis, db_id, xid,
                                                                            ddls);
}
...
RedisDDL::get_precommit_index_ddl()
{
    ...
    std::string precommit_key =
        fmt::format(redis::HASH_DDL_INDEX_PRECOMMIT, Properties::get_db_instance_id());
...
void
RedisDDL::commit_ddl(uint64_t db_id, uint64_t xid)
{
...
void
RedisDDL::abort_index_ddl(uint64_t db_id, uint64_t xid)
{
    std::string precommit_key =
        fmt::format(redis::HASH_DDL_INDEX_PRECOMMIT, Properties::get_db_instance_id());
    std::string hkey = fmt::format("{}:{}", db_id, xid);
...
RedisDDL::get_precommit_ddl()
{
    ...
    std::vector<std::string> hkeys;
    std::string precommit_key =
        fmt::format(redis::HASH_DDL_PRECOMMIT, Properties::get_db_instance_id());
    _redis->hkeys(precommit_key, std::back_inserter(hkeys));
...
void
RedisDDL::abort_ddl(uint64_t db_id, uint64_t xid)
{
    std::string precommit_key =
        fmt::format(redis::HASH_DDL_PRECOMMIT, Properties::get_db_instance_id());
    std::string hkey = fmt::format("{}:{}", db_id, xid);
...
void
RedisDDL::update_schema_xid(const std::string &fdw_id, uint64_t db_id, uint64_t schema_xid)
{
    ...
    std::string queue_key =
        fmt::format(redis::QUEUE_DDL_FDW, Properties::get_db_instance_id(), fdw_id);
