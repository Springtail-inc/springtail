...
## Key Components
  ...
  | **`HoleInfo`** | Struct holding an expired extent's location: `{ offset, size }` (`vacuumer.hh:207-210`) |
  | **`ExtentMap`** | Tracks expired extents: `file → xid → vector<HoleInfo>` (`vacuumer.hh:248`) |
  | **`SnapshotMap`** | Tracks expired snapshots: `db_id → xid → list<paths>` (`vacuumer.hh:255`) |
