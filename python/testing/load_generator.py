...
def load_data(config_file: str) -> None:
    ...
    while time.time() - start_time < RUN_TIME_SECONDS:
        ...
        tables = execute_sql(conn, f"SELECT tablename FROM pg_tables WHERE schemaname = '{schema}'")
        if tables:
            table = random.choice(tables)[0]
            insert_data(conn, schema, table)

        time.sleep(5)  # Delay before next cycle

        if len(created_schemas) > 2:
            drop_schema(conn, created_schemas.pop(0))
